...
noticias:API_BASE+"c_noticia.php",iNoticias:API_BASE+"i_noticia.php",uNoticias:API_BASE+"u_noticia.php",comentarios:API_BASE+"c_comentario_noticia.php",
tutores:API_BASE+"c_tutor.php",iTutores:API_BASE+"i_tutor.php",uTutores:API_BASE+"u_tutor.php",
prioridad:API_BASE+"c_prioridad.php",categorias:API_BASE+"c_categorias.php",calendario:API_BASE+"c_dias_curso.php",tipoEval:API_BASE+"c_tipo_evaluacion.php",actividades:API_BASE+"c_actividades.php",
...
const cacheGuard=o=>o&&(Date.now()-o._ts<30*60*1e3),arrToMap=arr=>{const m={};(Array.isArray(arr)?arr:[]).forEach(x=>m[x.id]=x.nombre);m._ts=Date.now();return m;};

let currentUser=getUsuarioFromCookie(),isAdminUser=ADMIN_IDS.includes(Number((currentUser&&currentUser.id)||0));

function getUsuarioFromCookie(){const row=(document.cookie||"").split("; ").find(r=>r.indexOf("usuario=")===0);if(!row)return null;try{const raw=row.split("=")[1]||"";const once=decodeURIComponent(raw);const maybe=/\%7B|\%22/.test(once)?decodeURIComponent(once):once;return JSON.parse(maybe)}catch(e){gcLog("cookie parse fail",e);return null}}
...
function onRouteChange(){
  ...
  const s=qs("#search-input"); if(s){ // placeholder dinámico
    if(hash.startsWith("#/cursos")) s.placeholder="Buscar por nombre o tutor";
    else if(hash.startsWith("#/noticias")) s.placeholder="Buscar por título";
    else if(hash.startsWith("#/tutores")) s.placeholder="Buscar por nombre o descripción";
    else if(hash.startsWith("#/usuarios")) s.placeholder="Buscar por nombre, correo o teléfono";
    else s.placeholder="Buscar…";
  }
...
function refreshCurrent(){if(state.route.startsWith("#/cursos"))return drawCursos();if(state.route.startsWith("#/noticias"))return drawNoticias();if(state.route.startsWith("#/tutores"))return drawTutores();if(state.route.startsWith("#/usuarios"))return drawUsuarios();if(state.route.startsWith("#/cuentas"))return drawCuentas()}
function defaultMatcher(q){const k=norm(q);return it=>norm(JSON.stringify(it)).includes(k)}
function renderList(rows,config){
  const d=qs("#recursos-list"),m=qs("#recursos-list-mobile");if(d)d.innerHTML="";if(m)m.innerHTML="";
  const filtered=state.search?rows.filter(config.matcher||defaultMatcher(state.search)):rows;
  if(!filtered.length){const empty="<div class='empty-state' style='padding:1rem;'>Sin resultados</div>";if(d)d.innerHTML=empty;if(m)m.innerHTML=empty;const c=qs("#mod-count");if(c)c.textContent="0 resultados";renderPagination(0);return}
  ...
  qsa("#recursos-list .table-row").forEach(el=>{
    el.addEventListener("click",()=>{const data=el.dataset||{};state.currentDrawer={type:data.type,id:Number(data.id),mode:"view"};openDrawer(config.drawerTitle(data),config.drawerBody(data));setTimeout(()=>config.afterOpen&&config.afterOpen(data),0)})
  });
  // mobile
  ...
  qsa("#recursos-list-mobile .open-drawer").forEach(btn=>{
    btn.addEventListener("click",e=>{e.stopPropagation();const host=btn.closest(".table-row-mobile");const data=(host&&host.dataset)||{};state.currentDrawer={type:data.type,id:Number(data.id),mode:"view"};openDrawer(config.drawerTitle(data),config.drawerBody(data));setTimeout(()=>config.afterOpen&&config.afterOpen(data),0)})
  });

  // Reactivar desde lista
  qsa(".gc-reactivate").forEach(btn=>{
    btn.addEventListener("click",async e=>{
      ...
      try{
        ...
        else if(t==="usuario"){await reactivateUsuario(id);toast("Usuario reactivado","exito");await loadUsuarios()}
...
fmtDate=d=>{if(!d)return"-";try{const p=String(d).split("-");return`${p[2]||""}/${p[1]||""}/${p[0]||""}`}catch{return d}},
fmtDateTime=dt=>{if(!dt)return"-";try{const sp=String(dt).split(" ");return`${fmtDate(sp[0])} ${sp[1]||""}`.trim()}catch{return dt}},
fmtMoney=n=>{try{return new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN"}).format(n)}catch{return"$"+n}},
...
statusSelect=(id,val)=>`<select id="${id}">${STATUS_OPTIONS.map(o=>`<option value="${o.v}"${Number(val)===Number(o.v)?" selected":""}>${o.l}</option>`).join("")}`,
withBust=url=>{try{const u=new URL(url,window.location.origin);u.searchParams.set("v",Date.now());return u.pathname+"?"+u.searchParams.toString()}catch{return url+(url.indexOf("?")>=0?"&":"?")+"v="+Date.now()}};

/* ===== Media (cursos/noticias/tutor) ===== */
...
function mountReadOnlyMedia(opt){
  const container=opt&&opt.container,type=opt&&opt.type,id=opt&&opt.id,labels=(opt&&opt.labels)||[];
  const editableOverride=opt&&opt.editable;if(!container)return;
  ...
  const urls=mediaUrlsByType(type,id);const grid=document.createElement("div");grid.className="media-grid";
  urls.forEach((url,i)=>{const label=labels[i]||("Imagen "+(i+1)),card=document.createElement("div");card.className="media-card";
    ...
    const img=card.querySelector("img");if(img){img.onerror=function(){img.onerror=null;if(type==="tutor"&&/\.png(\?|$)/.test(url)){img.src=withBust(url.replace(".png",".jpg"))}else{img.src="data:image/svg+xml;utf8,"+encodeURIComponent(noImageSvg())}}}
...
function bindUI(){
  qsa(".admin-dash .admin-nav, .gc-side .nav-item").forEach(btn=>{
    btn.addEventListener("click",function(){const route=btn.getAttribute("data-route")||btn.getAttribute("href");if(route){if(location.hash!==route)location.hash=route;else onRouteChange()}})
  });
  ...
  const addBtn=qs("#btn-add");if(addBtn)addBtn.addEventListener("click",async()=>{if(!isAdminUser)return;if(state.route.startsWith("#/cursos"))await openCreateCurso();else if(state.route.startsWith("#/noticias"))await openCreateNoticia();else if(state.route.startsWith("#/tutores"))await openCreateTutor();else if(state.route.startsWith("#/usuarios"))await openCreateUsuario()});
...
async function loadCursos(){
  ...
  const tt=qs(".tt-title");if(tt)tt.textContent="Cursos:";const st=qs("#tt-status");if(st){st.textContent="Activos e Inactivos";st.classList.remove("badge-inactivo");st.classList.add("badge-activo")}
  const s=qs("#search-input");if(s)s.placeholder="Buscar por nombre o tutor";
  showSkeletons();
  try{
...
function drawCursos(){
  ...
  renderList(rows,{matcher:q=>{const k=norm(q);return it=>norm(it.nombre).includes(k)||norm(it.tutor).includes(k)},
...
function renderCursoDrawer(dataset){
  const item=state.data.find(x=>String(x.id)===dataset.id),mode=(state.currentDrawer&&state.currentDrawer.mode)||(item?"view":"create"),isCreate=mode==="create"||!item,isEdit=mode==="edit",isView=mode==="view"&&!!item,c=isCreate?getEmptyCourse():(item?item._all:null);if(!c)return"<p>No encontrado.</p>";
  ...
  setTimeout(()=>{try{
    ...
    if(isCreate){const card=qs("#create-media-curso"),btn=qs("#create-media-edit"),thumb=qs("#create-media-thumb");if(btn&&thumb&&card){btn.addEventListener("click",()=>{const input=document.createElement("input");input.type="file";input.accept="image/png, image/jpeg";input.style.display="none";document.body.appendChild(input);input.addEventListener("change",()=>{const file=input.files&&input.files[0];try{document.body.removeChild(input)}catch{};if(!file)return;const v=validarImagen(file,{maxMB:2});if(!v.ok){toast(v.error,"error");return}renderPreviewUI(card,file,async()=>{state.tempNewCourseImage=file;try{if(thumb.dataset&&thumb.dataset.blobUrl)URL.revokeObjectURL(thumb.dataset.blobUrl)}catch{};const blobUrl=URL.createObjectURL(file);if(thumb.dataset)thumb.dataset.blobUrl=blobUrl;thumb.src=blobUrl;toast("Imagen seleccionada (se subirá al guardar)","exito")},()=>{})});input.click()})}}
...
async function loadNoticias(){
  ...
  const tt=qs(".tt-title");if(tt)tt.textContent="Noticias:";const st=qs("#tt-status");if(st){st.textContent="Publicadas e Inactivas";st.classList.remove("badge-inactivo");st.classList.add("badge-activo")}
  const s=qs("#search-input");if(s)s.placeholder="Buscar por título";
  showSkeletons();
  try{
    const [act,inact]=await Promise.all([postJSON(API.noticias,{estatus:1}),postJSON(API.noticias,{estatus:0})]);
    const arr=[].concat(Array.isArray(act)?act:[],Array.isArray(inact)?inact:[]);const counts=await Promise.all(arr.map(n=>getCommentsCount(n.id).catch(()=>0)));state.raw=arr;
...
function drawNoticias(){
  const rows=state.data;
  renderList(rows,{matcher:q=>{const k=norm(q);return it=>norm(it.titulo).includes(k)},
...
async function inactivateNoticia(id){const it=state.data.find(x=>x.id===Number(id));if(!it||!it._all)throw new Error("Noticia no encontrado");const body={...it._all,estatus:0};await postJSON(API.uNoticias,body)}
...
async function openCreateNoticia(){
  ...
  const estSel=statusSelect("f_estatus",1),imgCard=pos=>`
    <div class="media-card"><figure class="media-thumb">
      ...
      </button></figure><div class="media-meta"><div class="media-label">Imagen ${pos}</div><div class="media-help" style="color:#666;">JPG/PNG · Máx 2MB</div></div></div>`;
...
async function loadTutores(){
  ...
  const tt=qs(".tt-title");if(tt)tt.textContent="Tutores:";const st=qs("#tt-status");if(st){st.textContent="Activos e Inactivos";st.classList.remove("badge-inactivo");st.classList.add("badge-activo")}
  const s=qs("#search-input");if(s)s.placeholder="Buscar por nombre o descripción";
  showSkeletons();
  try{
...
function renderTutorDrawer(dataset){
  const item=state.data.find(x=>String(x.id)===dataset.id),mode=(state.currentDrawer&&state.currentDrawer.mode)||(item?"view":"create"),isCreate=mode==="create"||!item,isEdit=mode==="edit",isView=mode==="view"&&!!item,t=isCreate?{id:null,nombre:"",descripcion:"",estatus:1,creado_por:Number((currentUser&&currentUser.id)||0)||1}:(item?item._all:null);if(!t)return"<p>No encontrado.</p>";
...
async function loadUsuarios(){
  ...
  const tt=qs(".tt-title");if(tt)tt.textContent="Usuarios:";const st=qs("#tt-status");if(st){st.textContent="Activos e Inactivos";st.classList.remove("badge-inactivo");st.classList.add("badge-activo")}
  const s=qs("#search-input");if(s)s.placeholder="Buscar por nombre, correo o teléfono";
  showSkeletons();
  try{
    const [act,inact]=await Promise.all([postJSON(API.usuarios,{estatus:1}),postJSON(API.usuarios,{estatus:0})]);
    const raw=[].concat(Array.isArray(act)?act:[],Array.isArray(inact)?inact:[]);state.raw=raw;
    state.data=raw.map(u=>({id:u.id,nombre:u.nombre,correo:u.correo,telefono:u.telefono,estatus:ntf(u.estatus),_all:u}));
    drawUsuarios()
  }catch(err){gcLog(err);const list=qs("#recursos-list");if(list)list.innerHTML='<div style="padding:1rem;color:#b00020;">Error al cargar usuarios</div>';const m=qs("#recursos-list-mobile");if(m)m.innerHTML="";toast("No se pudieron cargar usuarios","error")}
}
function drawUsuarios(){
  const rows=state.data;
  renderList(rows,{matcher:q=>{const k=norm(q);return it=>norm(it.nombre||"").includes(k)||norm(it.correo||"").includes(k)||norm(it.telefono||"").includes(k)},
...
async function reactivateUsuario(id){const it=state.data.find(x=>x.id===Number(id));if(!it||!it._all)throw new Error("Usuario no encontrado");if(!API.uUsuarios)throw new Error("Endpoint u_usuario no disponible");await postJSON(API.uUsuarios,{...it._all,estatus:1})}
function readUsuarioForm(existing){const v=id=>(qs("#"+id)?.value||""),n=id=>Number(v(id)||0);const p={nombre:v("u_nombre"),correo:v("u_correo"),telefono:v("u_telefono"),fecha_nacimiento:v("u_fnac"),tipo_contacto:n("u_tcontacto"),estatus:n("u_estatus")};if(existing&&existing.id)p.id=existing.id;return p}
async function saveUsuarioUpdate(item){if(!item||!item._all)return toast("Sin item para actualizar","error");if(!API.uUsuarios)return toast("Endpoint u_usuario no disponible","warning");const p=readUsuarioForm(item._all);await postJSON(API.uUsuarios,{...item._all,...p});toast("Cambios guardados","exito");await loadUsuarios();const re=state.data.find(x=>x.id===item.id);if(re)openDrawer("Usuario · "+(re.nombre||re.correo||("#"+re.id)),renderUsuarioDrawer({id:String(re.id)}))}
async function saveUsuarioCreate(){if(!API.iUsuarios)return toast("Endpoint i_usuario no disponible","warning");const p=readUsuarioForm(null);if(!p.nombre||!p.correo||!p.telefono)return toast("Nombre, correo y teléfono son obligatorios","warning");await postJSON(API.iUsuarios,p);toast("Usuario creado","exito");closeDrawer();await loadUsuarios()}
...
function renderUsuarioDrawer(dataset){
  const it=state.data.find(x=>String(x.id)===dataset.id),n=it&&it._all;const mode=(state.currentDrawer&&state.currentDrawer.type==="usuario"&&state.currentDrawer.id===(n?.id))?state.currentDrawer.mode:"view",isEdit=mode==="edit",isView=!isEdit;if(!n)return"<p>No encontrado.</p>";
  const field=(label,val,input)=>`<div class="field"><div class="label">${escapeHTML(label)}</div><div class="value">${isEdit?input:escapeHTML(val!=null?val:"-")}</div></div>`;
...
function showCuentaPanel(){
  ...
  if(!qs("#cuenta-panel")){
    const host=qs(".main-content")||document.body;const panel=document.createElement("div");panel.id="cuenta-panel";panel.style.padding="16px 18px";
