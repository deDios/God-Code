...
function getUsuarioFromCookie(){
  ...
  try{
    ...
    const maybe=/\%7B|\%22/.test(once)?decodeURIComponent(once):once;
...
function showSkeletons(){
  ...
  for(let i=0;i<5;i++){
    t.insertAdjacentHTML("beforeend",'<div class="sk-row"><div class="sk n1"></div><div class="sk n2"></div><div class="sk n3"></div></div>');
  }
}
function renderPagination(total){
  ...
  [qs("#pagination-controls"),qs("#pagination-mobile")].forEach(cont=>{
    ...
    for(let p=1;p<=totalPages&&p<=7;p++){
      ...
      b.textContent=p; b.onclick=()=>{state.page=p; refreshCurrent();};
      cont.appendChild(b);
    }
...
function renderList(rows,config){
  ...
  qsa("#recursos-list-mobile .row-toggle").forEach(el=>{
    el.addEventListener("click",()=>{
      const row=el.closest(".table-row-mobile"); if(row&&row.classList) row.classList.toggle("expanded");
    });
  });
...
function mediaUrlsByType(type,id){
  ...
  if(type==="tutor")   return [`/ASSETS/tutor/tutor_${nid}.png`]; // fallback a .jpg en onerror
  return [];
}
function mountReadOnlyMedia(opt){
  const container=opt&&opt.container, type=opt&&opt.type, id=opt&&opt.id;
  ...
  urls.forEach((url,i)=>{
    ...
    const editBtnHTML= editable ? (
      ...
      '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1.0 1.0 0 0 0 0-1.41l-2.34-2.34a1.0 1.0 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" fill="currentColor"></path></svg></button>'
    ) : "";

    card.innerHTML =
      `<figure class="media-thumb"><img alt="${escapeAttr(label)}" src="${withBust(url)}">${editBtnHTML}</figure>`+
    ...
    if(img){
      img.onerror=function(){
        img.onerror=null;
        // Tutor fallback: si .png falla intenta .jpg
            ...
            renderPreviewUI(card,file,async()=>{
              try{
                if(type==="curso"){
                  ...
                  const res=await fetch(API_UPLOAD.cursoImg,{method:"POST",body:fd}); const text=await res.text().catch(()=> "");
                  if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                  let json; try{ json=JSON.parse(text) }catch{ json={_raw:text} }
                  img.src=withBust((json&&json.url)||url); toast("Imagen de curso actualizada","exito"); return;
                }
                if(type==="noticia"){
                  if(!API_UPLOAD.noticiaImg){ toast("Configura API_UPLOAD.noticiaImg","warning"); return; }
                  const fd=new FormData();
                  fd.append("noticia_id",String(id)); fd.append("pos",String(i+1)); fd.append("imagen",file);
                  const res=await fetch(API_UPLOAD.noticiaImg,{method:"POST",body:fd}); const text=await res.text().catch(()=> "");
                  if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                  let json; try{ json=JSON.parse(text) }catch{ json={_raw:text} }
                  img.src=withBust((json&&json.url)||url); toast("Imagen de noticia actualizada","exito"); return;
                }
                if(type==="tutor"){
                  ...
                  const res=await fetch(API_UPLOAD.tutorImg,{method:"POST",body:fd}); const text=await res.text().catch(()=> "");
                  if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                  let json; try{ json=JSON.parse(text) }catch{ json={_raw:text} }
                  img.src=withBust((json&&json.url)||url); toast("Imagen de tutor actualizada","exito"); return;
                }
  ...
  container.innerHTML =
    ...
    (editable?`<div class="media-help" style="color:#666;">Formatos: JPG/PNG · Máx 2MB</div>`:`<div class="media-help" style="color:#888;">Solo lectura</div>`)+
    `</div>`;
...
function renderPreviewUI(cardEl,file,onConfirm,onCancel){
  ...
  overlay.className="gc-preview-overlay"; overlay.setAttribute("role","dialog"); overlay.setAttribute("aria-modal","true");
  ...
  header.style.cssText="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:12px 16px;border-bottom:1px solid #eee;";
  header.innerHTML='<div style="font-weight:700;font-size:1.05rem;">Vista previa de imagen</div><button class="gc-btn gc-btn--ghost" data-act="close" aria-label="Cerrar" style="min-width:auto;padding:.35rem .6rem;">✕</button>';

  const body=document.createElement("div");
  body.style.cssText="display:grid;grid-template-columns:1fr 280px;gap:16px;padding:16px;align-items:start;";

  const imgWrap=document.createElement("div");
  imgWrap.style.cssText="border:1px solid #eee;border-radius:12px;padding:8px;background:#fafafa;display:flex;align-items:center;justify-content:center;min-height:320px;max-height:60vh;";
  ...
  const applyResponsive=()=>{ if(mql&&mql.matches){ body.style.gridTemplateColumns="1fr"; side.style.borderLeft="none"; side.style.paddingLeft="0"; imgWrap.style.minHeight="200px"; } else { body.style.gridTemplateColumns="1fr 280px"; side.style.borderLeft="1px dashed #e6e6e6"; side.style.paddingLeft="16px"; imgWrap.style.minHeight="320px"; } };
  if(mql&&mql.addEventListener) mql.addEventListener("change",applyResponsive); applyResponsive();

  body.appendChild(imgWrap); body.appendChild(side);
  ...
  const cleanup=()=>{
    if(drawer){
      if(drawer.style){ drawer.style.pointerEvents=prev.pe||""; drawer.style.filter=prev.fl||""; drawer.style.zIndex=prev.dz||""; }
      if(prev.aria!=null) drawer.setAttribute("aria-hidden",prev.aria); else drawer.removeAttribute("aria-hidden");
      try{ if(!prev.hadInert) drawer.removeAttribute("inert") }catch{}
    }
    if(overlayBG&&overlayBG.style) overlayBG.style.zIndex=prev.oz||"";
...
function validarImagen(file,opt){ opt=opt||{}; const maxMB=opt.maxMB||2;
  if(!file) return {ok:false,error:"No se seleccionó archivo"};
  const allowed=["image/jpeg","image/png"]; if(!allowed.includes(file.type)) return {ok:false,error:"Formato no permitido. Solo JPG o PNG"};
  const sizeMB=file.size/1024/1024; if(sizeMB>maxMB) return {ok:false,error:"La imagen excede "+maxMB+"MB"};
  return {ok:true};
}

/*  DRAWER BASE & JSON DEV  */
function openDrawer(title,bodyHTML){
  const ov=qs("#gc-dash-overlay"); if(ov&&ov.classList) ov.classList.add("open");
  const dw=qs("#gc-drawer"); if(!dw) return;
  const t=qs("#drawer-title"); if(t) t.textContent=title||"Detalle";
  const b=qs("#drawer-body"); if(b) b.innerHTML=bodyHTML||"";
  dw.classList&&dw.classList.add("open"); dw.setAttribute("aria-hidden","false");
}
function closeDrawer(){
  try{ if(document.activeElement&&document.activeElement.blur) document.activeElement.blur(); }catch{}
  const ov=qs("#gc-dash-overlay"); if(ov&&ov.classList) ov.classList.remove("open");
  const dw=qs("#gc-drawer"); if(!dw) return;
  dw.classList&&dw.classList.remove("open"); dw.setAttribute("aria-hidden","true");
  state.currentDrawer=null; gcLog("closeDrawer");
}
...
async function loadCursos(){
  ...
  if(hdr){
    ...
    if(!c4){ const nc=document.createElement("div"); nc.className="col-status"; nc.setAttribute("role","columnheader"); nc.textContent="Status"; hdr.appendChild(nc); }
  ...
  try{
    ...
    state.data=raw.map(c=>({
      ...
      tipo_eval_id:c.tipo_evaluacion, tipo_eval_nombre:(temap&&temap[c.tipo_evaluacion])||("#"+c.tipo_evaluacion),
      actividades_id:c.actividades, actividades_nombre:(ammap&&ammap[c.actividades])||("#"+c.actividades),
      precio:c.precio, certificado:!!c.certificado,
      fecha:c.fecha_inicio, estatus:ntf(c.estatus),
      _all:c
    }));
    drawCursos();
  }catch(err){
    gcLog(err);
    const list=qs("#recursos-list"); if(list) list.innerHTML='<div style="padding:1rem;color:#b00020;">Error al cargar cursos</div>';
  ...
  renderList(rows,{
    ...
    desktopRow:(it)=>`
      <div class="table-row" data-id="${it.id}" data-type="curso">
        <div class="col-nombre"><span class="name-text">${escapeHTML(it.nombre)}</span> ${badgePrecio(it.precio)}</div>
    ...
    afterOpen:(d)=>{
      if(d.type==="curso"){
        const it=state.data.find(x=>String(x.id)===d.id); if(!it) return;
        const cont=qs("#media-curso");
        if(cont) mountReadOnlyMedia({container:cont,type:"curso",id:it.id,labels:["Portada"],editable:isAdminUser&&(state.currentDrawer&&state.currentDrawer.mode==="edit")});
...
function normalizeCursoPayload(p){
  return {
    ...p,
    nombre:String(p.nombre||""), descripcion_breve:String(p.descripcion_breve||""), descripcion_curso:String(p.descripcion_curso||""),
    descripcion_media:String(p.descripcion_media||""), dirigido:String(p.dirigido||""), competencias:String(p.competencias||""),
    certificado:Number(!!p.certificado), tutor:Number(p.tutor||0), horas:Number(p.horas||0), precio:Number(p.precio||0),
    estatus:Number(p.estatus!=null?p.estatus:1),
    prioridad:Number(p.prioridad||1), categoria:Number(p.categoria||1), calendario:Number(p.calendario||1),
    tipo_evaluacion:Number(p.tipo_evaluacion||1), actividades:Number(p.actividades||1),
    creado_por:Number(p.creado_por||0), fecha_inicio:String(p.fecha_inicio||"")
  };
}
function mapToOptions(map,selectedId){
  if(!map||typeof map!=="object") return '<option value="">—</option>';
  const pairs=Object.keys(map).filter(k=>k!=="_ts").map(k=>[k,map[k]]);
  if(!pairs.length) return '<option value="">—</option>';
  return pairs.map(([id,name])=>`<option value="${escapeAttr(id)}"${String(selectedId)===String(id)?" selected":""}>${escapeHTML(name)}</option>`).join("");
}
function getEmptyCourse(){
  return { nombre:"", descripcion_breve:"", descripcion_curso:"", descripcion_media:"", dirigido:"", competencias:"",
    certificado:0, tutor:"", horas:0, precio:0, estatus:1, fecha_inicio:"",
    prioridad:1, categoria:1, calendario:1, tipo_evaluacion:1, actividades:1,
    creado_por: Number((currentUser&&currentUser.id)||0)||1 };
}
function readCursoForm(existingId){
  const read=id=>{ const el=qs("#"+id); return el?el.value:""; };
...
async function saveNewCurso(){
  ...
  if(newId&&file){
    try{ await uploadCursoImagen(newId,file); toast("Imagen subida","exito"); }
    catch(err){ gcLog(err); toast("Curso creado, pero falló la imagen","error"); }
    finally{ state.tempNewCourseImage=null; }
  }

  toast("Curso creado","exito"); closeDrawer();
  await loadCursos();
...
async function openCreateCurso(){
  try{
    await Promise.all([getTutorsMap(),getPrioridadMap(),getCategoriasMap(),getCalendarioMap(),getTipoEvalMap(),getActividadesMap()]);
    state.currentDrawer={type:"curso",id:null,mode:"create"};
    openDrawer("Curso · Crear",renderCursoDrawer({id:""}));
  }catch(e){ gcLog(e); toast("No se pudo abrir el formulario","error"); }
}
  ...
  setTimeout(()=>{
    try{
          ...
          btn.addEventListener("click",()=>{
            ...
            input.addEventListener("change",()=>{
              ...
              renderPreviewUI(card,file,async()=>{
                ...
                try{ if(thumb.dataset&&thumb.dataset.blobUrl) URL.revokeObjectURL(thumb.dataset.blobUrl); }catch{}
      ...
      if(btnSave) btnSave.addEventListener("click",async(e)=>{
        e.stopPropagation();
        try{ if(isCreate) await saveNewCurso(); else await saveUpdateCurso(item); }
      ...
      if(btnCancel) btnCancel.addEventListener("click",(e)=>{
        e.stopPropagation();
        if(isCreate){ state.tempNewCourseImage=null; closeDrawer(); }
        else{
      ...
      if(bDel) bDel.addEventListener("click",async(e)=>{
        ...
        try{ await softDeleteCurso(item); toast("Curso eliminado (inactivo)","exito"); closeDrawer(); await loadCursos(); }
        catch(err){ gcLog(err); toast("No se pudo eliminar","error"); }
      });

      const btnRea=qs("#btn-reactivar");
      if(btnRea) btnRea.addEventListener("click",async(e)=>{
        e.stopPropagation();
        try{
          await reactivateCurso(Number(item&&item.id)); toast("Curso reactivado","exito"); await loadCursos();
      ...
      if(contCurso){
        ...
        if(!isNaN(cid)&&cid){
          mountReadOnlyMedia({container:contCurso,type:"curso",id:cid,labels:["Portada"],editable:isEdit&&isAdminUser});
        }
      }
      if(isAdminUser) bindCopyFromPre("#json-curso","#btn-copy-json-curso");
...
async function getCommentsCount(nid){
  const r=await postJSON(API.comentarios,{noticia_id:Number(nid),estatus:1});
...
async function loadNoticias(){
  ...
  try{
    ...
    state.raw=arr;
    state.data=arr.map((n,i)=>({ id:n.id, titulo:n.titulo, fecha:n.fecha_creacion, estatus:ntf(n.estatus), comentarios:counts[i]||0, _all:n }));
    drawNoticias();
  }catch(err){
    gcLog(err);
    const list=qs("#recursos-list"); if(list) list.innerHTML='<div style="padding:1rem;color:#b00020;">Error al cargar noticias</div>';
    const m=qs("#recursos-list-mobile"); if(m) m.innerHTML="";
  ...
  renderList(rows,{
    ...
    mobileRow:(it)=>`
      <div class="table-row-mobile" data-id="${it.id}" data-type="noticia">
        <button class="row-toggle">
          <div class="col-nombre">${escapeHTML(it.titulo)}</div><span class="icon-chevron">›</span>
        </button>
        <div class="row-details">
    ...
    afterOpen:(d)=>{
      if(d.type==="noticia"){
        const nid=Number(d.id), cont=qs("#media-noticia");
        if(cont) mountReadOnlyMedia({container:cont,type:"noticia",id:nid,labels:["Imagen 1","Imagen 2"],editable:isAdminUser&&(state.currentDrawer&&state.currentDrawer.mode==="edit")});
...
async function inactivateNoticia(id){
  const it=state.data.find(x=>x.id===Number(id)); if(!it||!it._all) throw new Error("Noticia no encontrada");
  const body={...it._all,estatus:0}; await postJSON(API.uNoticias,body);
}
async function reactivateNoticia(id){
  const it=state.data.find(x=>x.id===Number(id)); if(!it||!it._all) throw new Error("Noticia no encontrada");
  if(!API.uNoticias){ toast("Falta endpoint u_noticia.php en backend","warning",3500); return false; }
  const body={...it._all,estatus:1}; await postJSON(API.uNoticias,body); return true;
}
function readNoticiaForm(n){
  const g=id=>{ const el=qs("#"+id); return el?el.value:""; };
  const gN=(id,def)=>Number(g(id)||def||0);
  return { id:n.id, titulo:g("f_tit"), desc_uno:g("f_desc1"), desc_dos:g("f_desc2"), estatus:gN("f_estatus",1), creado_por:n.creado_por };
}
function renderNoticiaDrawer(dataset){
  const item=state.data.find(x=>String(x.id)===dataset.id);
  const n=item&&item._all; if(!n) return "<p>No encontrado.</p>";
  ...
  setTimeout(()=>{
    try{
      // montar media SIEMPRE (view y edit)
      const cont=qs("#media-noticia");
      if(cont) mountReadOnlyMedia({container:cont,type:"noticia",id:n.id,labels:["Imagen 1","Imagen 2"],editable:isEdit&&isAdminUser});

      const be=qs("#btn-edit");
      if(be) be.addEventListener("click",(e)=>{ e.stopPropagation(); state.currentDrawer={type:"noticia",id:n.id,mode:"edit"}; qs("#drawer-body").innerHTML=renderNoticiaDrawer({id:String(n.id)}); });

      const bc=qs("#btn-cancel");
      if(bc) bc.addEventListener("click",(e)=>{ e.stopPropagation(); state.currentDrawer={type:"noticia",id:n.id,mode:"view"}; qs("#drawer-body").innerHTML=renderNoticiaDrawer({id:String(n.id)}); });
      ...
      if(bDel) bDel.addEventListener("click",async(e)=>{
        ...
        try{ await inactivateNoticia(n.id); toast("Noticia eliminada (inactiva)","exito"); closeDrawer(); await loadNoticias(); }
        catch(err){ gcLog(err); toast("No se pudo eliminar","error"); }
      });

      const br=qs("#btn-reactivar");
      if(br) br.addEventListener("click",async(e)=>{
        e.stopPropagation();
        const ok=await reactivateNoticia(n.id);
        if(ok){ toast("Noticia reactivada","exito"); await loadNoticias(); const re=state.data.find(x=>x.id===n.id); if(re) openDrawer("Noticia · "+re.titulo,renderNoticiaDrawer({id:String(re.id)})); }
      });
...
async function openCreateNoticia(){
  // Drawer simple de creación con 2 slots de imagen
  ...
  const html=(()=>{
    ...
    const imgCard=(pos)=>`
      <div class="media-card">
        <figure class="media-thumb">
          <img id="create-news-thumb-${pos}" alt="Imagen ${pos}" src="${withBust(`/ASSETS/noticia/NoticiasImg/noticia_img${pos}_0.png`)}"/>
          <button class="icon-btn media-edit" id="create-news-edit-${pos}" title="Seleccionar imagen">
            <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1.0 1.0 0 0 0 0-1.41l-2.34-2.34a1.0 1.0 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" fill="currentColor"></path></svg>
          </button>
        </figure>
    ...
    return ''+
      ...
      `<div class="field"><div class="label">Imágenes</div>
        <div class="value"><div id="create-media-noticia" class="media-grid">${imgCard(1)}${imgCard(2)}</div></div>
      </div>`;
  })();
  ...
  setTimeout(()=>{
      ...
      if(btn&&img&&wrap){
        btn.addEventListener("click",()=>{
          ...
          input.addEventListener("change",()=>{
            ...
            if(!file) return;
            const v=validarImagen(file,{maxMB:2}); if(!v.ok){ toast(v.error,"error"); return; }
            renderPreviewUI(wrap,file,async()=>{
              state.tempNewNewsImages[pos]=file;
              try{ if(img.dataset&&img.dataset.blobUrl) URL.revokeObjectURL(img.dataset.blobUrl); }catch{}
              const blobUrl=URL.createObjectURL(file);
    ...
    const btnCancel=qs("#btn-cancel");
    if(btnCancel) btnCancel.addEventListener("click",()=>{ state.tempNewNewsImages={1:null,2:null}; closeDrawer(); });
    ...
    if(btnSave) btnSave.addEventListener("click",async()=>{
      try{
        ...
        const payload={
          titulo: (qs("#f_tit")?.value||"").trim(),
          desc_uno: (qs("#f_desc1")?.value||"").trim(),
          desc_dos: (qs("#f_desc2")?.value||"").trim(),
          estatus: ntf(qs("#f_estatus")?.value||1),
          creado_por: Number((currentUser&&currentUser.id)||0)||1
        };
        ...
        for(const pos of [1,2]){
          ...
          if(newId && f){
            try{
              const fd=new FormData();
              fd.append("noticia_id",String(newId));
              fd.append("pos",String(pos));
              fd.append("imagen",f);
              await fetch(API_UPLOAD.noticiaImg,{method:"POST",body:fd});
            }catch(e){ gcLog(e); toast(`Noticia creada, pero falló imagen ${pos}`,"error"); }
          }
        }
        state.tempNewNewsImages={1:null,2:null};
        toast("Noticia creada","exito"); closeDrawer();
        await loadNoticias();
...
async function loadTutores(){
  ...
  if(hdr){
    const c1=hdr.querySelector(".col-nombre"),
          ...
          c4=hdr.querySelector(".col-status");
    if(c1) c1.textContent="Nombre";
    if(c2){ c2.textContent="Descripción"; c2.classList.add("col-tipo"); }
    if(c3) c3.textContent=""; // ya no mostramos fecha
    if(c4) c4.textContent="Status";
  }
  ...
  renderList(rows,{
    ...
    desktopRow:(it)=>`
      <div class="table-row" data-id="${it.id}" data-type="tutor">
        ...
        <div class="col-tutor" title="${escapeAttr(it.descripcion||"")}">${escapeHTML((it.descripcion||"").slice(0,60))}${(it.descripcion||"").length>60?"…":""}</div>
        <div class="col-fecha"></div>
        <div class="col-status">${statusBadge(it.estatus)}</div>
      </div>`,
    mobileRow:(it)=>`
      <div class="table-row-mobile" data-id="${it.id}" data-type="tutor">
        <button class="row-toggle"><div class="col-nombre">${escapeHTML(it.nombre)}</div><span class="icon-chevron">›</span></button>
        <div class="row-details">
    ...
    afterOpen:(d)=>{
      if(d.type==="tutor"){
        ...
        if(cont) mountReadOnlyMedia({container:cont,type:"tutor",id:it.id,labels:["Foto"],editable:isAdminUser&&(state.currentDrawer&&state.currentDrawer.mode==="edit")});
...
function readTutorForm(existing){
  const g=id=>{ const el=qs("#"+id); return el?el.value:""; };
  const gN=(id,def)=>Number(g(id)||def||0);
  const p={ nombre:g("f_nombre"), descripcion:g("f_desc"), estatus:gN("f_estatus",1) };
...
async function saveTutorUpdate(item){
  if(!item||!item._all) return toast("Sin item para actualizar","error");
  const p=readTutorForm(item._all); const body={...item._all,...p};
  await postJSON(API.uTutores,body);
  toast("Cambios guardados","exito"); await loadTutores();
  const re=state.data.find(x=>x.id===item.id);
  if(re) openDrawer("Tutor · "+re.nombre,renderTutorDrawer({id:String(re.id)}));
}
async function softDeleteTutor(item){
  if(!item||!item._all) throw new Error("Item inválido");
  const body={...item._all,estatus:0}; await postJSON(API.uTutores,body);
}
async function reactivateTutor(id){
  const it=state.data.find(x=>x.id===Number(id));
  if(!it||!it._all) throw new Error("Tutor no encontrado");
  const body={...it._all,estatus:1}; await postJSON(API.uTutores,body);
}
...
function renderTutorDrawer(dataset){
  ...
  const mode=(state.currentDrawer&&state.currentDrawer.mode)||(item?"view":"create");
  const isCreate=mode==="create"||!item, isEdit=mode==="edit", isView=mode==="view"&&!!item;
  const t=isCreate?{id:null,nombre:"",descripcion:"",estatus:1,creado_por:Number((currentUser&&currentUser.id)||0)||1}:(item?item._all:null);
  ...
  setTimeout(()=>{
    try{
      ...
      if(btnSave) btnSave.addEventListener("click",async(e)=>{
        e.stopPropagation();
        try{ if(isCreate) await saveTutorCreate(); else await saveTutorUpdate(item); }
        catch(err){ gcLog(err); toast("Error al guardar","error"); }
      });
      ...
      if(btnEdit) btnEdit.addEventListener("click",(e)=>{
        ...
        qs("#drawer-body").innerHTML=renderTutorDrawer({id:String(item?item.id:"")});
      });
      ...
      if(bDel) bDel.addEventListener("click",async(e)=>{
        ...
        try{ await softDeleteTutor(item); toast("Tutor eliminado (inactivo)","exito"); closeDrawer(); await loadTutores(); }
        catch(err){ gcLog(err); toast("No se pudo eliminar","error"); }
      });
      ...
      if(btnRea) btnRea.addEventListener("click",async(e)=>{
        e.stopPropagation();
        try{
          await reactivateTutor(Number(item&&item.id)); toast("Tutor reactivado","exito"); await loadTutores();
          const re=state.data.find(x=>x.id===(item&&item.id)); if(re) openDrawer("Tutor · "+re.nombre,renderTutorDrawer({id:String(re.id)}));
        }catch(err){ gcLog(err); toast("No se pudo reactivar","error"); }
      });

      const cont=qs("#media-tutor");
      if(cont && t.id) mountReadOnlyMedia({container:cont,type:"tutor",id:t.id,labels:["Foto"],editable:isEdit&&isAdminUser});
      if(isAdminUser) bindCopyFromPre("#json-tutor","#btn-copy-json-tutor");
...
/*  CUENTAS (placeholder enriquecido)  */
