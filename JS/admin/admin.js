...
function enforceRouteGuard(){
  if(!isAdminUser){ const h=(window.location.hash||"").toLowerCase(); if(h.indexOf("#/cuentas")!==0){ if(location.hash!=="#/cuentas") location.hash="#/cuentas"; } }
}
...
function renderList(rows,config){
  ...
  if(!filtered.length){
    ...
    if(d) d.innerHTML=empty; if(m) m.innerHTML=empty; const c=qs("#mod-count"); if(c) c.textContent="0 resultados"; renderPagination(0); return;
  }
  ...
  qsa("#recursos-list .table-row").forEach(el=>{
    el.addEventListener("click",()=>{ const data=el.dataset||{}; state.currentDrawer={type:data.type,id:Number(data.id),mode:"view"};
      openDrawer(config.drawerTitle(data),config.drawerBody(data)); setTimeout(()=>config.afterOpen&&config.afterOpen(data),0);
    });
  });
  ...
  qsa("#recursos-list-mobile .open-drawer").forEach(btn=>{
    btn.addEventListener("click",(e)=>{ e.stopPropagation(); const host=btn.closest(".table-row-mobile"); const data=(host&&host.dataset)||{};
      state.currentDrawer={type:data.type,id:Number(data.id),mode:"view"}; openDrawer(config.drawerTitle(data),config.drawerBody(data)); setTimeout(()=>config.afterOpen&&config.afterOpen(data),0);
    });
  });
  ...
  qsa(".gc-reactivate").forEach(btn=>{
    btn.addEventListener("click",async(e)=>{
      e.stopPropagation();
      const id=Number(btn.getAttribute("data-id")); const t=btn.getAttribute("data-type");
      try{
...
function renderPreviewUI(cardEl,file,onConfirm,onCancel){
  ...
  const modal=document.createElement("div"); modal.className="gc-preview-modal";
  ...
  const header=document.createElement("div"); header.style.cssText="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:12px 16px;border-bottom:1px solid #eee;";
  ...
  mql&&mql.addEventListener&&mql.addEventListener("change",applyResponsive); applyResponsive();
  ...
  const cleanup=()=>{ if(drawer){ if(drawer.style){drawer.style.pointerEvents=prev.pe||"";drawer.style.filter=prev.fl||"";drawer.style.zIndex=prev.dz||"";} if(prev.aria!=null)drawer.setAttribute("aria-hidden",prev.aria); else drawer.removeAttribute("aria-hidden"); try{if(!prev.hadInert)drawer.removeAttribute("inert")}catch{} }
    if(overlayBG?.style) overlayBG.style.zIndex=prev.oz||""; document.body.style.overflow=""; try{URL.revokeObjectURL(url)}catch{}; try{overlay.remove()}catch{}; document.removeEventListener("keydown",onEsc);};
...
function mountReadOnlyMedia(opt){
  const container=opt?.container, type=opt?.type, id=opt?.id, labels=opt?.labels||[]; const editableOverride=opt?.editable;
  if(!container) return;
  ...
  urls.forEach((url,i)=>{
    ...
    if(img){ img.onerror=function(){ img.onerror=null;
        if(type==="tutor" && /\.png(\?|$)/.test(url)){ img.src=withBust(url.replace(".png",".jpg")); }
        else if(type==="usuario"){ img.src=withBust(getDefaultAvatar()); }
            ...
            renderPreviewUI(card,file,async()=>{
              try{
                if(type==="curso"){
                  ...
                  const res=await fetch(API_UPLOAD.cursoImg,{method:"POST",body:fd}); const text=await res.text().catch(()=> ""); if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                ...
                if(type==="noticia"){
                  ...
                  const res=await fetch(API_UPLOAD.noticiaImg,{method:"POST",body:fd}); const text=await res.text().catch(()=> ""); if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                ...
                if(type==="tutor"){
                  ...
                  const res=await fetch(API_UPLOAD.tutorImg,{method:"POST",body:fd}); const text=await res.text().catch(()=> ""); if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                ...
                if(type==="usuario"){
                  const fd=new FormData(); fd.append("usuario_id",String(id)); fd.append("imagen",file);
                  const res=await fetch(API.uAvatar,{method:"POST",body:fd}); const text=await res.text().catch(()=> ""); if(!res.ok) throw new Error("HTTP "+res.status+" "+(text||""));
                  let json; try{json=JSON.parse(text)}catch{json={_raw:text}}; img.src=withBust((json&&json.url)||url); toast("Avatar actualizado","exito"); return;
                }
...
async function openCreateNoticia(){
  ...
  setTimeout(()=>{const pick=(pos)=>{const btn=qs(`#create-news-edit-${pos}`), img=qs(`#create-news-thumb-${pos}`), wrap=qs("#create-media-noticia");
    if(btn&&img&&wrap){btn.addEventListener("click",()=>{const input=document.createElement("input"); input.type="file"; input.accept="image/png, image/jpeg"; input.style.display="none"; document.body.appendChild(input);
      input.addEventListener("change",()=>{const file=input.files&&input.files[0]; try{document.body.removeChild(input);}catch{} if(!file) return; const v=validarImagen(file,{maxMB:2}); if(!v.ok){toast(v.error,"error"); return;}
        renderPreviewUI(wrap,file,async()=>{state.tempNewNewsImages[pos]=file; try{if(img.dataset?.blobUrl) URL.revokeObjectURL(img.dataset.blobUrl);}catch{} const blobUrl=URL.createObjectURL(file); if(img.dataset) img.dataset.blobUrl=blobUrl; img.src=blobUrl; toast(`Imagen ${pos} lista (se subirá al guardar)`,"exito");},()=>{});
...
function readUsuarioForm(existing){const v=id=>qs("#"+id)?.value||""; const n=id=>Number(v(id)||0);
  const p={nombre:v("u_nombre"),correo:v("u_correo"),telefono:v("u_telefono"),fecha_nacimiento:v("u_fnac"),tipo_contacto:n("u_tcontacto"),estatus:n("u_estatus")};
  if(existing?.id) p.id=existing.id; return p;
}
