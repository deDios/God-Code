...
const state={
  ...
  cursosMap:null,usuariosMap:null
};
...
function getUsuarioFromCookie(){
  ...
  if(!row)return null;
  try{
    const raw=row.split("=")[1]||"",once=decodeURIComponent(raw);
...
async function postJSON(url,body){
  ...
  if(!r.ok)throw new Error("HTTP "+r.status+" "+(t||""));if(!t.trim())return {};
...
async function fetchAllUsuariosAnyStatus(){
  ...
  const [e1,e0,e2,e3]=await Promise.all([
    postJSON(API.usuarios,{estatus:1}),postJSON(API.usuarios,{estatus:0}),
    postJSON(API.usuarios,{estatus:2}),postJSON(API.usuarios,{estatus:3})
  ]);
...
async function fetchAllCursosAnyStatus(){
  ...
  const [e1,e0,e2,e3]=await Promise.all([
    ...
    postJSON(API.cursos,{estatus:2}),postJSON(API.cursos,{estatus:3})
  ]);
  const arr=[].concat(Array.isArray(e1)?e1:[],Array.isArray(e0)?e0:[],Array.isArray(e2)?e2:[],Array.isArray(e3)?e3:[]);
...
function applyAdminVisibility(isAdmin){
  qsa(".gc-side .nav-item").forEach(a=>{if(!isAdmin&&!isCuentasLink(a)){(a.closest&&a.closest("li")?a.closest("li"):a).style.display="none";a.setAttribute("tabindex","-1");a.setAttribute("aria-hidden","true")}});
...
function renderList(rows,config){
  ...
  const matcherFn=(config&&typeof config.matcher==="function")?config.matcher(state.search):defaultMatcher(state.search);
  const filtered=state.search?rows.filter(matcherFn):rows;
...
function renderPreviewUI(cardEl,file,onConfirm,onCancel){
  ...
  const prev={pe:drawer?.style?.pointerEvents,fl:drawer?.style?.filter,dz:drawer?.style?.zIndex,oz:overlayBG?.style?.zIndex,aria:drawer?drawer.getAttribute("aria-hidden"):null,hadInert:drawer?.hasAttribute?drawer.hasAttribute("inert"):false};
...
function mountReadOnlyMedia(opt){
  const container=opt?.container,type=opt?.type,id=opt?.id,labels=opt?.labels||[];const editableOverride=opt?.editable;if(!container)return;
...
async function loadCursos(){
  ...
  try{
    ...
    state.data=(raw||[]).map(c=>({id:c.id,nombre:c.nombre,tutor:(tmap&&tmap[c.tutor])||("Tutor #"+c.tutor),tutor_id:c.tutor,
      ...
      precio:c.precio,certificado:!!c.certificado,fecha:c.fecha_inicio,estatus:ntf(c.estatus),_all:c}));
    drawCursos()
...
async function openCreateNoticia(){
  ...
  const html='<div class="gc-actions"><button class="gc-btn gc-btn--ghost" id="btn-cancel">Cancelar</button><button class="gc-btn gc-btn--primary" id="btn-save">Guardar</button></div>'+
    ...
    '<div class="field"><div class="label">Descripción (2)</div><div class="value"><textarea id="f_desc2" rows="3" placeholder="Texto adicional"></textarea></div></div>'+
...
async function saveTutorCreate(){
  ...
  const file=state.tempNewTutorImage||null;if(!file)return toast("Selecciona una imagen para el tutor","warning");
  ...
  try{
    if(newId&&file){const v=validarImagen(file,{maxMB:2});if(!v.ok)return toast(v.error,"error");const fd=new FormData();fd.append("tutor_id",String(newId));fd.append("imagen",file);const r=await fetch(API_UPLOAD.tutorImg,{method:"POST",body:fd});if(!r.ok){const tt=await r.text();throw new Error(tt||"upload fail")}}
...
function renderTutorDrawer(dataset){
  ...
  let html=""+controls+
    ...
    field("Estatus",t.estatus,statusSelect("f_estatus",t.estatus));
  if(isCreate){
  ...
  if(!isCreate){
    html+=`
      <div class="field"><div class="label">Cursos activos</div><div class="value"><div id="tutor-cursos-activos" class="tutor-cursos"></div></div></div>
      <div class="field"><div class="label">Cursos pausados</div><div class="value"><div id="tutor-cursos-pausados" class="tutor-cursos"></div></div></div>
      <div class="field"><div class="label">Cursos terminados</div><div class="value"><div id="tutor-cursos-terminados" class="tutor-cursos"></div></div></div>`;
  }
...
async function loadUsuarios(){
  ...
  try{
    const [e1,e0,e2,e3]=await Promise.all([postJSON(API.usuarios,{estatus:1}),postJSON(API.usuarios,{estatus:0}),postJSON(API.usuarios,{estatus:2}),postJSON(API.usuarios,{estatus:3})]);
    const arr=[].concat(Array.isArray(e1)?e1:[],Array.isArray(e0)?e0:[],Array.isArray(e2)?e2:[],Array.isArray(e3)?e3:[]);
...
function renderUsuarioDrawer(dataset){
  const it=state.data.find(x=>String(x.id)===dataset.id);const n=it&&it._all;const mode=(state.currentDrawer&&state.currentDrawer.type==="usuario"&&state.currentDrawer.id===(n?.id))?state.currentDrawer.mode:"view";
  const isEdit=mode==="edit",isView=!isEdit;if(!n)return"<p>No encontrado.</p>";
  const field=(label,val,input)=>`<div class="field"><div class="label">${escapeHTML(label)}</div><div class="value">${isEdit?input:escapeHTML(val!=null?val:"-")}</div></div>`;
  ...
  let html=controls+
   ...
   `<div class="field"><div class="label">Avatar</div><div class="value"><div id="media-usuario" data-id="${n.id}"></div></div></div>`;
...
async function openCreateUsuario(){
  ...
  `<div class="field"><div class="label">Avatar</div><div class="value"><div id="create-media-usuario" class="media-grid"><div class="media-card"><figure class="media-thumb"><img id="create-user-avatar" alt="Avatar" src="${withBust(getDefaultAvatar())}"/><button class="icon-btn media-edit" id="create-user-avatar-btn" title="Seleccionar avatar"><svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1.0 1.0 0 0 0 0-1.41l-2.34-2.34a1.0 1.0 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" fill="currentColor"></path></svg></button></figure><div class="media-meta"><div class="media-label">Avatar</div><div class="media-help" style="color:#666;">JPG/PNG · Máx 2MB</div></div></div></div></div></div>`;
...
async function loadSuscripciones(){
  ...
  if(hdr){
    ...
    if(c1)c1.textContent="Suscriptor";if(c2){c2.textContent="Curso";c2.classList.add("col-tipo")}if(c3)c3.textContent="Fecha de suscripción";if(c4)c4.textContent="Status"
  }
  ...
  try{
    const [e1,e0,e2,e3,cMap,uMap]=await Promise.all([
      postJSON(API.inscripciones,{estatus:1}),postJSON(API.inscripciones,{estatus:0}),
      postJSON(API.inscripciones,{estatus:2}),postJSON(API.inscripciones,{estatus:3}),
      getCursosMapAnyStatus(),getUsuariosMapAnyStatus()
    ]);
...
function drawSuscripciones(){
  const rows=state.data;
  renderList(rows,{matcher:q=>{const k=norm(q);return it=>norm(it.suscriptor).includes(k)||norm(it.curso_nombre).includes(k)},
...
function readSuscripcionForm(existing){const g=id=>qs("#"+id)?.value||"";const n=id=>Number(g(id)||0);const p={usuario:n("s_usuario"),curso:n("s_curso"),comentario:g("s_comentario"),estatus:n("s_estatus")};if(existing?.id)p.id=existing.id;return p}
...
async function openCreateSuscripcion(){
  ...
  const [cursosMap,usuariosMap]=await Promise.all([getCursosMapAnyStatus(),getUsuariosMapAnyStatus()]);
  const sel=(id,map)=>`<select id="${id}">${Object.keys(map).filter(k=>k!=="_ts").map(id=>`<option value="${escapeAttr(id)}">${escapeHTML(map[id])}</option>`).join("")}</select>`;
  const html='<div class="gc-actions"><button class="gc-btn gc-btn--ghost" id="btn-cancel">Cancelar</button><button class="gc-btn gc-btn--primary" id="btn-save">Guardar</button></div>'+
    `<div class="field"><div class="label">Usuario</div><div class="value">${sel("s_usuario",usuariosMap)}</div></div>`+
    `<div class="field"><div class="label">Curso</div><div class="value">${sel("s_curso",cursosMap)}</div></div>`+
    `<div class="field"><div class="label">Estatus</div><div class="value">${statusSelect("s_estatus",1)}</div></div>`+
    `<div class="field"><div class="label">Comentario</div><div class="value"><textarea id="s_comentario" rows="3" placeholder="Comentario (opcional)"></textarea></div></div>`;
  state.currentDrawer={type:"suscripcion",id:null,mode:"create"};
  openDrawer("Suscripción · Crear",html);
  setTimeout(()=>{
    disableDrawerInputs(false);
    qs("#btn-cancel")?.addEventListener("click",()=>closeDrawer());
    qs("#btn-save")?.addEventListener("click",async()=>{
      try{
        const p=readSuscripcionForm(null);
        if(!p.usuario||!p.curso)return toast("Selecciona usuario y curso","warning");
        const res=await postJSON(API.iInscripcion,{usuario:p.usuario,curso:p.curso,comentario:p.comentario||"",estatus:ntf(p.estatus||1)});
        const newId=Number((res&&(res.id||res.suscripcion_id||res.insert_id||(res.data&&res.data.id)))||0);
        toast("Suscripción creada","exito");closeDrawer();await loadSuscripciones();
        if(newId){const re=state.data.find(x=>x.id===newId);if(re)openDrawer("Suscripción · "+re.curso_nombre,renderSuscripcionDrawer({id:String(re.id)}))}
      }catch(err){gcLog(err);toast("Error al crear suscripción","error")}
    });
  },0)
}
