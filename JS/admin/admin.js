(() => {
  // ------------------------------------- Debug
  window.GC_DEBUG = false; // true/false para ver logs
  function gcLog(...a) {
  ...
  const API_UPLOAD = {
    ...
    // noticiaImg: "https://.../u_noticiaImg.php" // cuando lo tengas
  };

  // ---- ids de usuarios con permisos de admin
  ...
  function fmtDate(d) {
    if (!d) return "-";
    try { const p = String(d).split("-"); return (p[2] || "") + "/" + (p[1] || "") + "/" + (p[0] || ""); } catch { return d; }
  }
  function fmtDateTime(dt) {
    if (!dt) return "-";
    try { const sp = String(dt).split(" "); return (fmtDate(sp[0]) + " " + (sp[1] || "")).trim(); } catch { return dt; }
  }
  function fmtMoney(n) {
  ...
  // ---- Panel de cuenta (mostrar/ocultar)
  function showCuentaPanel() {
    try { const el = qs(".recursos-box.desktop-only"); if (el) el.style.display = "none"; } catch { }
    try { const el2 = qs(".recursos-box.mobile-only"); if (el2) el2.style.display = "none"; } catch { }
    try { const p1 = qs("#pagination-controls"); if (p1) p1.style.display = "none"; } catch { }
    try { const p2 = qs("#pagination-mobile"); if (p2) p2.style.display = "none"; } catch { }
  ...
  function hideCuentaPanel() {
    const panel = qs("#cuenta-panel"); if (panel) panel.remove();
  ...
  async function getTutorsMap() {
    ...
    const map = {}; (Array.isArray(arr) ? arr : []).forEach(t => { map[t.id] = t.nombre; });
    map._ts = Date.now(); state.tutorsMap = map; return map;
  }
  async function getPrioridadMap() {
    ...
    const map = {}; (Array.isArray(arr) ? arr : []).forEach(p => { map[p.id] = p.nombre; });
    map._ts = Date.now(); state.prioMap = map; return map;
  }
  async function getCategoriasMap() {
    ...
    const map = {}; (Array.isArray(arr) ? arr : []).forEach(c => { map[c.id] = c.nombre; });
    map._ts = Date.now(); state.categoriasMap = map; return map;
  }
  async function getCalendarioMap() {
    ...
    const map = {}; (Array.isArray(arr) ? arr : []).forEach(c => { map[c.id] = c.nombre; });
    map._ts = Date.now(); state.calendarioMap = map; return map;
  }
  async function getTipoEvalMap() {
    ...
    const map = {}; (Array.isArray(arr) ? arr : []).forEach(c => { map[c.id] = c.nombre; });
    map._ts = Date.now(); state.tipoEvalMap = map; return map;
  }
  async function getActividadesMap() {
    ...
    const map = {}; (Array.isArray(arr) ? arr : []).forEach(c => { map[c.id] = c.nombre; });
    map._ts = Date.now(); state.actividadesMap = map; return map;
  }
  ...
  // Chip simple: "good" -> badge activo; resto -> inactivo
  ...
  function setRoute(hash) {
    ...
    if (location.hash !== target) location.hash = target; else onRouteChange();
  }
  ...
  function showSkeletons() {
    const d = qs("#recursos-list"); const m = qs("#recursos-list-mobile");
    if (d) d.innerHTML = ""; if (m) m.innerHTML = "";
    const target = d || m; if (!target) return;
    for (let i = 0; i < 5; i++) {
  ...
  function renderList(rows, config) {
    ...
    if (!rows.length) {
      ...
      const countEl = qs("#mod-count"); if (countEl) countEl.textContent = "0 resultados";
    ...
    qsa("#recursos-list .table-row").forEach((el) => {
      el.addEventListener("click", function () {
        ...
        // montar media en Noticia si aplica
        if (data.type === "noticia") {
        ...
        if (data.type === "tutor") {
          setTimeout(() => { if (isAdminUser) bindCopyFromPre("#json-tutor", "#btn-copy-json-tutor"); }, 0);
        }
      });
    });
    ...
    // Botones Reactivar (lista) — cursos/noticias
    qsa(".gc-reactivate").forEach((btn) => {
      btn.addEventListener("click", async function (e) {
        ...
        try {
          if (t === "curso") {
            await reactivateCurso(id); toast("Curso reactivado", "exito"); await loadCursos();
          } else if (t === "noticia") {
            const ok = await reactivateNoticia(id);
            if (ok) { toast("Noticia reactivada", "exito"); await loadNoticias(); }
          }
  ...
  function refreshCurrent() {
    ...
    if (state.route.indexOf("#/cursos/suscripciones") === 0) return; // placeholder
  ...
  async function loadCursos() {
    ...
    if (hdr) {
      ...
      if (!c4) {
        ...
        c4.className = "col-status"; c4.setAttribute("role", "columnheader"); c4.textContent = "Status";
        hdr.appendChild(c4);
    ...
    if (ttStatus) { ttStatus.textContent = "Activos e Inactivos"; ttStatus.classList.remove("badge-inactivo"); ttStatus.classList.add("badge-activo"); }
    ...
    try {
      ...
      const raw = [].concat(Array.isArray(activosRaw) ? activosRaw : [], Array.isArray(inactivosRaw) ? inactivosRaw : []);
      state.raw = raw;
      ...
      gcLog(err); toast("No se pudieron cargar cursos", "error");
    }
  }
  ...
  function drawCursos() {
    ...
    renderList(rows, {
      ...
      <div class="table-row" data-id="${it.id}" data-type="curso">
        ...
        <div class="col-status">${badgeGeneric('curso', it.estatus)}</div>
      </div>`,
      mobileRow: (it) => `
  ...
  function badgePrecio(precio) {
    return Number(precio) === 0 ? '<span class="gc-chip gray">Gratuito</span>' : '<span class="gc-chip gray">Con costo</span>';
  }
  ...
  function normalizeCursoPayload(p) {
    return {
      ...
      estatus: Number(p.estatus != null ? p.estatus : 1), // desde el form
  ...
  function renderCursoDrawer(dataset) {
    ...
    const inTA = (id, val, rows) => `<textarea id="${id}" rows="${rows || 4}">${escapeHTML(val || "")}</textarea>`;
    ...
    const field = (label, value, inputHTML) =>
      `<div class="field"><div class="label">${escapeHTML(label)}</div><div class="value">${(isEdit || isCreate) ? inputHTML : escapeHTML(value != null ? value : "-")}</div></div>`;

    // acciones
    let controlsRow = "";
    if (isCreate) {
      controlsRow = '<div class="gc-actions"><button class="gc-btn gc-btn--ghost" id="btn-cancel">Cancelar</button><button class="gc-btn gc-btn--primary" id="btn-save">Guardar</button></div>';
    } else if (isAdminUser) {
    ...
    setTimeout(function () {
      try {
        ...
        if (btnSave) btnSave.addEventListener("click", async function (e) {
          e.stopPropagation();
        ...
        if (btnRea) btnRea.addEventListener("click", async function (e) {
          e.stopPropagation();
          try {
        ...
        if (contCurso) {
          ...
          if (!isNaN(cid) && cid) {
            mountReadOnlyMedia({
              container: contCurso, type: "curso", id: cid,
              labels: ["Portada"], editable: isEdit && isAdminUser,
            });
          }
        }
  ...
  function getEmptyCourse() {
    return {
      ...
      dirigido: "", competencias: "", certificado: 0, tutor: "", horas: 0, precio: 0,
      estatus: 1, fecha_inicio: "", prioridad: 1, categoria: 1, calendario: 1,
      tipo_evaluacion: 1, actividades: 1, creado_por: Number((currentUser && currentUser.id) || 0) || 1,
    };
  }
  ...
  function readCursoForm(existingId) {
    ...
    const payload = {
      ...
      estatus: readN("f_estatus", 1), // desde <select>
  ...
  async function saveNewCurso() {
    ...
    if (!newId) { gcLog("Respuesta de iCursos sin id utilizable:", res); toast("Curso creado, pero no se recibió ID", "warning"); }

    const fileToUpload = state.tempNewCourseImage || null;
    if (newId && fileToUpload) {
      try { await uploadCursoImagen(newId, fileToUpload); toast("Imagen subida", "exito"); }
      catch (err) { gcLog(err); toast("Curso creado, pero falló la subida de imagen", "error"); }
  ...
  async function saveUpdateCurso(item) {
    ...
    if (f) {
      try { await uploadCursoImagen(item.id, f); toast("Imagen actualizada", "exito"); }
      catch (err) { gcLog(err); toast("Se guardó el curso, pero falló la imagen", "error"); }
    }
  ...
  async function loadNoticias() {
    ...
    try {
      ...
      const raw = [].concat(Array.isArray(activos) ? activos : [], Array.isArray(inactivos) ? inactivos : []);
      state.raw = raw;
      ...
      mobileRow: (it) => `
        <div class="table-row-mobile" data-id="${it.id}" data-type="noticia">
          ...
          <div class="row-details">
            ...
            <div style="display:flex;gap:8px;margin:.25rem 0;">
              <button class="gc-btn gc-btn--ghost open-drawer">Ver detalle</button>
  ...
  function renderNoticiaDrawer(dataset) {
    ...
    // incluye contenedor para imágenes
    const html =
      ...
      `<div class="field"><div class="label">Imágenes</div><div class="value"><div id="media-noticia"></div></div></div>` +
      jsonSection(n, "JSON · Noticia", "json-noticia", "btn-copy-json-noticia");

    return html;
  }

  // Opcional: reactivar noticia (inactivo -> activo)
  async function reactivateNoticia(id) {
    const it = state.data.find(x => x.id === Number(id));
    if (!it || !it._all) throw new Error("Noticia no encontrada");
    const body = normalizeNoticiaPayload({ ...it._all, estatus: 1 });
    await postJSON(API.uNoticias, body);
    return true;
  }
  function normalizeNoticiaPayload(p) {
    return {
      id: Number(p.id || 0),
      titulo: String(p.titulo || ""),
      desc_uno: String(p.desc_uno || ""),
      desc_dos: String(p.desc_dos || ""),
  ...
  function loadSuscripciones() {
    ...
    if (list) list.innerHTML = `
      <div style="padding:2rem; text-align:center;">
        ...
        <button class="gc-btn" onclick="location.hash='#/cursos'">Volver a Cursos</button>
      </div>`;
    const m = qs("#recursos-list-mobile");
    if (m) m.innerHTML = "";
    const countEl = qs("#mod-count"); if (countEl) countEl.textContent = "—";
  }
  ...
  function drawCuentas() {
    ...
    // El panel real se inyecta con showCuentaPanel()
    const list = qs("#recursos-list"); if (list) list.innerHTML = "";
    const m = qs("#recursos-list-mobile"); if (m) m.innerHTML = "";
    const countEl = qs("#mod-count"); if (countEl) countEl.textContent = "";
  }
  ...
  function ensureDrawerHost() {
    if (!qs("#gc-dash-overlay")) {
      ...
      ov.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.35);opacity:0;visibility:hidden;transition:opacity .2s ease, visibility .2s ease;z-index:9998;";
    ...
    if (!qs("#gc-drawer")) {
      ...
      dr.style.cssText = "position:fixed;top:0;right:0;height:100vh;width:min(720px,92vw);background:#fff;box-shadow:-8px 0 24px rgba(0,0,0,.18);transform:translateX(100%);transition:transform .25s ease;z-index:9999;display:flex;flex-direction:column;";
      dr.innerHTML = `
  ...
  function closeDrawer() {
    ...
    // limpiar body para evitar listeners colgados
    setTimeout(() => { const b = qs("#drawer-body"); if (b) b.innerHTML = ""; }, 200);
  }

  // ---------- Helpers de JSON / Copiado ----------
  ...
  function bindCopyFromPre(preSel, btnSel) {
    const btn = qs(btnSel); const pre = qs(preSel);
    if (!btn || !pre) return;
    btn.addEventListener("click", async function (e) {
      e.preventDefault();
      const text = pre.textContent || "";
      if (!text) return toast("No hay JSON para copiar", "warning");
      try { await copyText(text); toast("JSON copiado", "exito"); }
      catch {
        try {
  ...
  function renderPreviewUI(cardEl, file, onConfirm, onCancel) {
    ...
    const cleanup = function () {
      if (drawer) {
        ...
        if (prev.drawerAria != null) drawer.setAttribute("aria-hidden", prev.drawerAria);
        else drawer.removeAttribute("aria-hidden");
        try { if (!prev.hadInert) drawer.removeAttribute("inert"); } catch { }
      }
      if (drawerOverlay && drawerOverlay.style) drawerOverlay.style.zIndex = prev.overlayZ || "";
    ...
    const onEsc = function (e) { if (e && e.key === "Escape") { e.preventDefault(); cleanup(); } };
    ...
    const btnCancel = side.querySelector('[data-act="cancel"]'); if (btnCancel) btnCancel.addEventListener("click", function (e) { e.preventDefault(); onCancel && onCancel(); cleanup(); });
    const btnConfirm = side.querySelector('[data-act="confirm"]'); if (btnConfirm) btnConfirm.addEventListener("click", async function (e) { e.preventDefault(); try { if (onConfirm) await onConfirm(); } finally { cleanup(); } });
  }

  // ---- Imágenes (lectura + edición en cursos si está en modo edición)
  function mountReadOnlyMedia(opt) {
    ...
    const editable =
      typeof editableOverride === "boolean" ? editableOverride
    ...
    urls.forEach(function (url, i) {
      ...
      card.innerHTML =
        '<figure class="media-thumb"><img alt="' + escapeAttr(label) + '" src="' + withBust(url) + '">' +
        editBtnHTML + "</figure>" +
  ...
  // ---------- Toolbar: añadir (solo cursos por ahora) ----------
  function bindToolbar() {
    ...
    addBtn.onclick = () => {
      if (state.route.indexOf("#/cursos") === 0) {
        ...
        toast("La creación rápida está disponible sólo en Cursos por ahora.", "warning");
      }
    };
    addBtn.style.display = isAdminUser ? "" : "none";
  }

  // ---------- INIT ----------
  document.addEventListener("DOMContentLoaded", () => {
    currentUser = getUsuarioFromCookie();
    isAdminUser = currentUser && currentUser.id && ADMIN_IDS.indexOf(currentUser.id) >= 0;
