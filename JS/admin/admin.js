(() => {
  "use strict";
  /* ==== CONFIG & UTILS ==== */
  const GC_DEBUG = !1, gcLog = (...a) => { if (GC_DEBUG && typeof console != "undefined") { try { console.log("[GC]", ...a) } catch (e) { } } }, qs = (s, r = document) => r.querySelector(s), qsa = (s, r = document) => Array.prototype.slice.call(r.querySelectorAll(s)), toast = (m, t = "exito", d = 2500) => window.gcToast ? window.gcToast(m, t, d) : gcLog(`[${t}] ${m}`);
  ...
  const API_BASE = "https://godcode-dqcwaceacpf2bfcd.mexicocentral-01.azurewebsites.net/db/web/",
    ...
    API_UPLOAD = { cursoImg: API_BASE + "u_cursoImg.php", noticiaImg: API_BASE + "u_noticiaImagenes.php", tutorImg: API_BASE + "u_tutorImagen.php" };
  const ADMIN_IDS = [1, 12, 13], STATUS_OPTIONS = [{ v: 1, l: "Activo" }, { v: 0, l: "Inactivo" }, { v: 2, l: "Borrador" }, { v: 3, l: "Archivado" }],
    state = { route: "#/cursos", page: 1, pageSize: 10, raw: [], data: [], search: "", currentDrawer: null, tempNewCourseImage: null, tutorsMap: null, prioMap: null, categoriasMap: null, calendarioMap: null, tipoEvalMap: null, actividadesMap: null },
    getUsuarioFromCookie = () => { const row = (document.cookie || "").split("; ").find(r => r.indexOf("usuario=") === 0); if (!row) return null; const raw = row.split("=")[1] || ""; try { const once = decodeURIComponent(raw); const maybeTwice = /%7B|%22/.test(once) ? decodeURIComponent(once) : once; return JSON.parse(maybeTwice) } catch (e) { gcLog("cookie parse fail", e); return null } };
  let currentUser = getUsuarioFromCookie(), isAdminUser = ADMIN_IDS.includes(Number((currentUser && currentUser.id) || 0));
  const ntf = s => Number(s || 0),
    postJSON = async (url, body) => { gcLog("postJSON ->", url, body); const r = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body || {}) }); const t = await r.text().catch(() => { return "" }); gcLog("postJSON <-", url, r.status, t); if (!r.ok) throw new Error("HTTP " + r.status + " " + (t || "")); if (!t.trim()) return {}; try { return JSON.parse(t) } catch (e) { return { _raw: t } } };

  /* ==== CATALOGS (cursos) with cache ==== */
  const cacheGuard = obj => obj && Date.now() - obj._ts < 18e5, arrToMap = arr => { const m = {}; (Array.isArray(arr) ? arr : []).forEach(x => { m[x.id] = x.nombre }); m._ts = Date.now(); return m };
  ...
  function showCuentaPanel() { try { const el = qs(".recursos-box.desktop-only"); if (el && el.style) el.style.display = "none" } catch (e) { } try { const el2 = qs(".recursos-box.mobile-only"); if (el2 && el2.style) el2.style.display = "none" } catch (e) { } try { const p = qs("#pagination-controls"); if (p && p.style) p.style.display = "none" } catch (e) { } try { const p2 = qs("#pagination-mobile"); if (p2 && p2.style) p2.style.display = "none" } catch (e) { } if (!qs("#cuenta-panel")) { const host = qs(".main-content") || document.body; const panel = document.createElement("div"); panel.id = "cuenta-panel"; panel.style.padding = "16px 18px"; panel.innerHTML = window.renderCuentaOpciones ? window.renderCuentaOpciones() : "<div>Panel de cuenta</div>"; host.appendChild(panel) } }
  ...
  function renderList(rows, config) {
    ...
    qsa("#recursos-list .table-row").forEach(el => { el.addEventListener("click", function () { const data = el.dataset || {}; state.currentDrawer = { type: data.type, id: Number(data.id), mode: "view" }; openDrawer(config.drawerTitle(data), config.drawerBody(data)); setTimeout(() => { config.afterOpen && config.afterOpen(data) }, 0) }) });
    qsa("#recursos-list-mobile .row-toggle").forEach(el => { el.addEventListener("click", function () { const row = el.closest(".table-row-mobile"); row && row.classList && row.classList.toggle("expanded") }) });
  ...
  /* ==== FORMAT & HELPERS ==== */
  const escapeHTML = s => String(s == null ? "" : s).replace(/[&<>'"]/g, s => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" }[s])), escapeAttr = s => String(s == null ? "" : s).replace(/"/g, "&quot;"),
    fmtDate = d => { if (!d) return "-"; try { const p = String(d).split("-"); return `${p[2] || ""}/${p[1] || ""}/${p[0] || ""}` } catch (e) { return d } },
    fmtDateTime = dt => { if (!dt) return "-"; try { const sp = String(dt).split(" "); return `${fmtDate(sp[0])} ${sp[1] || ""}`.trim() } catch (e) { return dt } },
    fmtMoney = n => { try { return new Intl.NumberFormat("es-MX", { style: "currency", currency: "MXN" }).format(n) } catch (e) { return "$" + n } },
    pair = (l, v) => `<div class="field"><div class="label">${escapeHTML(l)}</div><div class="value">${escapeHTML(v != null ? v : "-")}</div></div>`,
    statusBadge = v => Number(v) === 1 ? '<span class="gc-badge-activo">Activo</span>' : Number(v) === 0 ? '<span class="gc-badge-inactivo">Inactivo</span>' : `<span class="gc-badge-gray">Estatus ${escapeHTML(v)}</span>`,
    statusText = v => { const f = STATUS_OPTIONS.find(x => Number(x.v) === Number(v)); return f ? f.l : `Estatus ${v}` },
    statusSelect = (id, val) => `<select id="${id}">${STATUS_OPTIONS.map(o => `<option value="${o.v}"${Number(val) === Number(o.v) ? " selected" : ""}>${o.l}</option>`).join("")}</select>`,
    withBust = url => { try { const u = new URL(url, window.location.origin); u.searchParams.set("v", Date.now()); return u.pathname + "?" + u.searchParams.toString() } catch (e) { return url + (url.indexOf("?") >= 0 ? "&" : "?") + "v=" + Date.now() } };
  ...
  function mediaUrlsByType(type, id) { const nid = Number(id); if (type === "noticia") return [`/ASSETS/noticia/NoticiasImg/noticia_img1_${nid}.png`, `/ASSETS/noticia/NoticiasImg/noticia_img2_${nid}.png`]; if (type === "curso") return [`/ASSETS/cursos/img${nid}.png`]; if (type === "tutor") return [`/ASSETS/tutor/tutor_${nid}.png`, `/ASSETS/tutor/tutor_${nid}.jpg`]; return [] }
  function mountReadOnlyMedia(opt) {
    const container = opt && opt.container, type = opt && opt.type, id = opt && opt.id, labels = (opt && opt.labels) || [], editableOverride = opt && opt.editable; if (!container) return;
    const editable = typeof editableOverride === "boolean" ? editableOverride : (isAdminUser && state.currentDrawer && state.currentDrawer.mode === "edit"), urls = mediaUrlsByType(type, id);
    const grid = document.createElement("div"); grid.className = "media-grid";
    urls.forEach((url, i) => {
      const label = labels[i] || "Imagen " + (i + 1); const card = document.createElement("div"); card.className = "media-card";
      ...
      const img = card.querySelector("img"); if (img) img.onerror = function () { img.onerror = null; img.src = "data:image/svg+xml;utf8," + encodeURIComponent(noImageSvg()) };
      if (editable) {
  ...
  function renderPreviewUI(cardEl, file, onConfirm, onCancel) {
    const url = URL.createObjectURL(file), drawer = qs("#gc-drawer"), drawerOverlay = qs("#gc-dash-overlay"),
      prev = { pe: drawer && drawer.style ? drawer.style.pointerEvents : "", fl: drawer && drawer.style ? drawer.style.filter : "", dz: drawer && drawer.style ? drawer.style.zIndex : "", oz: drawerOverlay && drawerOverlay.style ? drawerOverlay.style.zIndex : "", aria: drawer ? drawer.getAttribute("aria-hidden") : null, hadInert: drawer && typeof drawer.hasAttribute === "function" ? drawer.hasAttribute("inert") : !1 };
    ...
    if (drawer) { drawer.style.pointerEvents = "none"; drawer.style.filter = "blur(1px)"; drawer.style.zIndex = "1"; drawer.setAttribute("aria-hidden", "true"); try { drawer.setAttribute("inert", "") } catch (e) { } }
    ...
    side.innerHTML = `<div style="font-weight:600;">Detalles</div><div style="font-size:.92rem;color:#444;line-height:1.35;"><div><strong>Archivo:</strong> ${file.name}</div><div><strong>Peso:</strong> ${humanSize(file.size)}</div><div><strong>Tipo:</strong> ${file.type || "desconocido"}</div><div style="margin-top:6px;color:#666;">Formatos permitidos: JPG / PNG · Máx 2MB</div></div><div style="margin-top:auto;display:flex;gap:8px;flex-wrap:wrap;"><button class="gc-btn gc-btn--primary" data-act="confirm">Subir</button><button class="gc-btn gc-btn--ghost" data-act="cancel">Cancelar</button></div>`;
    const mql = window.matchMedia && window.matchMedia("(max-width: 720px)"), applyResponsive = () => { if (mql && mql.matches) { body.style.gridTemplateColumns = "1fr"; side.style.borderLeft = "none"; side.style.paddingLeft = "0"; imgWrap.style.minHeight = "200px" } else { body.style.gridTemplateColumns = "1fr 280px"; side.style.borderLeft = "1px dashed #e6e6e6"; side.style.paddingLeft = "16px"; imgWrap.style.minHeight = "320px" } }; if (mql && mql.addEventListener) mql.addEventListener("change", applyResponsive); applyResponsive();
    ...
    const cleanup = () => { if (drawer) { if (drawer.style) { drawer.style.pointerEvents = prev.pe || ""; drawer.style.filter = prev.fl || ""; drawer.style.zIndex = prev.dz || "" } if (prev.aria != null) drawer.setAttribute("aria-hidden", prev.aria); else drawer.removeAttribute("aria-hidden"); try { if (!prev.hadInert) drawer.removeAttribute("inert") } catch (e) { } } if (drawerOverlay && drawerOverlay.style) drawerOverlay.style.zIndex = prev.oz || ""; document.body.style.overflow = ""; try { URL.revokeObjectURL(url) } catch (e) { } try { overlay.remove() } catch (e) { } document.removeEventListener("keydown", onEsc) };
    const onEsc = e => { if (e && e.key === "Escape") { e.preventDefault(); cleanup() } }; document.addEventListener("keydown", onEsc);
    overlay.addEventListener("click", e => { if (e.target === overlay) cleanup() });
    const btnClose = header.querySelector('[data-act="close"]'); if (btnClose) btnClose.addEventListener("click", cleanup);
    const btnCancel = side.querySelector('[data-act="cancel"]'); if (btnCancel) btnCancel.addEventListener("click", e => { e.preventDefault(); onCancel && onCancel(); cleanup() });
    const btnConfirm = side.querySelector('[data-act="confirm"]'); if (btnConfirm) btnConfirm.addEventListener("click", async e => { e.preventDefault(); try { if (onConfirm) await onConfirm() } finally { cleanup() } })
  }
  function validarImagen(file, opt) { opt = opt || {}; const maxMB = opt.maxMB || 2; if (!file) return { ok: !1, error: "No se seleccionó archivo" }; const allowed = ["image/jpeg", "image/png"]; if (!allowed.includes(file.type)) return { ok: !1, error: "Formato no permitido. Solo JPG o PNG" }; const sizeMB = file.size / 1048576; if (sizeMB > maxMB) return { ok: !1, error: "La imagen excede " + maxMB + "MB" }; return { ok: !0 } }
  ...
  function bindCopyFromPre(preSel, btnSel) { const btn = qs(btnSel), pre = qs(preSel); if (!btn || !pre) return; btn.addEventListener("click", async e => { e.preventDefault(); const text = pre.textContent || ""; if (!text) return toast("No hay JSON para copiar", "warning"); try { await navigator.clipboard.writeText(text); toast("JSON copiado", "exito") } catch (e) { try { const ta = document.createElement("textarea"); ta.value = text; ta.style.position = "fixed"; ta.style.left = "-9999px"; document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta); toast("JSON copiado", "exito") } catch (e) { alert("No se pudo copiar") } } }) }

  /* ==== TOOLBAR / SEARCH / BUTTONS ==== */
  function bindUI() {
    qsa(".admin-dash .admin-nav, .gc-side .nav-item").forEach(btn => { btn.addEventListener("click", function () { const route = btn.getAttribute("data-route") || btn.getAttribute("href"); if (route) { if (location.hash !== route) location.hash = route; else onRouteChange() } }) });
    const drawerClose = qs("#drawer-close"); if (drawerClose) drawerClose.addEventListener("click", closeDrawer);
    ...
    const addBtn = qs("#btn-add"); if (addBtn) addBtn.addEventListener("click", async () => { if (!isAdminUser) return; if (state.route.startsWith("#/cursos")) { await openCreateCurso() } else if (state.route.startsWith("#/noticias")) { await openCreateNoticia() } else if (state.route.startsWith("#/tutores")) { await openCreateTutor() } });
  ...
  function badgeCurso(estatus) { return statusBadge(estatus) }
  ...
  function renderCursoDrawer(dataset) {
    ...
    const inText = (id, v, ph) => `<input id="${id}" type="text" value="${escapeAttr(v || "")}" placeholder="${escapeAttr(ph || "")}" />`,
      inNum = (id, v, min) => `<input id="${id}" type="number" value="${escapeAttr(v != null ? v : "")}" min="${min || "0"}" />`,
      inDate = (id, v) => `<input id="${id}" type="date" value="${escapeAttr(v || "")}" />`,
      inCheck = (id, val) => `<label class="gc-inline"><input id="${id}" type="checkbox" ${Number(val) ? "checked" : ""}/> <span>Sí</span></label>`,
      inSel = (id, opts) => `<select id="${id}">${opts}</select>`,
      inTA = (id, v, rows) => `<textarea id="${id}" rows="${rows || 4}">${escapeHTML(v || "")}</textarea>`;
    const tutorOptions = mapToOptions(state.tutorsMap, String(c.tutor || "")),
      prioOptions = mapToOptions(state.prioMap, String(c.prioridad || "")),
      catOptions = mapToOptions(state.categoriasMap, String(c.categoria || "")),
      calOptions = mapToOptions(state.calendarioMap, String(c.calendario || "")),
      tipoOptions = mapToOptions(state.tipoEvalMap, String(c.tipo_evaluacion || "")),
      actOptions = mapToOptions(state.actividadesMap, String(c.actividades || ""));
    ...
    let controls = "";
    if (isCreate) { controls = '<div class="gc-actions"><button class="gc-btn gc-btn--ghost" id="btn-cancel">Cancelar</button><button class="gc-btn gc-btn--primary" id="btn-save">Guardar</button></div>' }
    else if (isAdminUser) { const isInactive = Number(c.estatus) === 0; controls = '<div class="gc-actions">' + (isView ? '<button class="gc-btn" id="btn-edit">Editar</button>' : "") + (isEdit ? '<button class="gc-btn gc-btn--ghost" id="btn-cancel">Cancelar</button>' : "") + (isEdit ? '<button class="gc-btn gc-btn--primary" id="btn-save">Guardar</button>' : "") + '<button class="gc-btn gc-btn--danger" id="btn-delete" data-step="1">Eliminar</button>' + (isInactive ? '<button class="gc-btn gc-btn--success" id="btn-reactivar">Reactivar</button>' : "") + "</div>" }
    let html = "" + controls
    ...
    setTimeout(() => {
      try {
        ...
        if (isCreate) { const card = qs("#create-media-curso"), btn = qs("#create-media-edit"), thumb = qs("#create-media-thumb"); if (btn && thumb && card) { btn.addEventListener("click", function () { const input = document.createElement("input"); input.type = "file"; input.accept = "image/png, image/jpeg"; input.style.display = "none"; document.body.appendChild(input); input.addEventListener("change", function () { const file = input.files && input.files[0]; try { document.body.removeChild(input) } catch (e) { } if (!file) return; const v = validarImagen(file, { maxMB: 2 }); if (!v.ok) { toast(v.error, "error"); return } renderPreviewUI(card, file, async () => { state.tempNewCourseImage = file; try { if (thumb.dataset && thumb.dataset.blobUrl) URL.revokeObjectURL(thumb.dataset.blobUrl) } catch (e) { } const blobUrl = URL.createObjectURL(file); if (thumb.dataset) thumb.dataset.blobUrl = blobUrl; thumb.src = blobUrl; toast("Imagen seleccionada (se subirá al guardar)", "exito") }, () => { }) }); input.click() }) } }
        const btnSave = qs("#btn-save"); if (btnSave) btnSave.addEventListener("click", async e => { e.stopPropagation(); try { if (isCreate) await saveNewCurso(); else await saveUpdateCurso(item) } catch (err) { gcLog(err); toast("Error al guardar", "error") } });

        const btnEdit = qs("#btn-edit"); if (btnEdit) btnEdit.addEventListener("click", e => { e.stopPropagation(); state.currentDrawer = { type: "curso", id: item ? item.id : null, mode: "edit" }; qs("#drawer-body").innerHTML = renderCursoDrawer({ id: String(item ? item.id : "") }) });
        const btnCancel = qs("#btn-cancel"); if (btnCancel) btnCancel.addEventListener("click", e => { e.stopPropagation(); if (isCreate) { state.tempNewCourseImage = null; closeDrawer() } else { state.currentDrawer = { type: "curso", id: item ? item.id : null, mode: "view" }; qs("#drawer-body").innerHTML = renderCursoDrawer({ id: String(item ? item.id : "") }) } });
        const bDel = qs("#btn-delete"); if (bDel) bDel.addEventListener("click", async e => { e.stopPropagation(); const step = bDel.getAttribute("data-step") || "1"; if (step === "1") { bDel.textContent = "Confirmar"; bDel.setAttribute("data-step", "2"); setTimeout(() => { if (bDel.getAttribute("data-step") === "2") { bDel.textContent = "Eliminar"; bDel.setAttribute("data-step", "1") } }, 4e3); return } try { await softDeleteCurso(item); toast("Curso eliminado (inactivo)", "exito"); closeDrawer(); await loadCursos() } catch (err) { gcLog(err); toast("No se pudo eliminar", "error") } });
        const btnRea = qs("#btn-reactivar"); if (btnRea) btnRea.addEventListener("click", async e => { e.stopPropagation(); try { await reactivateCurso(Number(item && item.id)); toast("Curso reactivado", "exito"); await loadCursos(); const re = state.data.find(x => x.id === (item && item.id)); if (re) openDrawer("Curso · " + re.nombre, renderCursoDrawer({ id: String(re.id) })) } catch (err) { gcLog(err); toast("No se pudo reactivar", "error") } });
  ...
  async function loadNoticias() {
    ...
    const tt = qs(".tt-title"); if (tt) tt.textContent = "Noticias:"; const ttStatus = qs("#tt-status"); if (ttStatus) { ttStatus.textContent = "Publicadas e Inactivas"; ttStatus.classList.remove("badge-inactivo"); ttStatus.classList.add("badge-activo") }
    showSkeletons();
    try {
      ...
      const counts = await Promise.all(arr.map(n => getCommentsCount(n.id).catch(() => { return 0 })));
  ...
  async function reactivateNoticia(id) { const it = state.data.find(x => x.id === Number(id)); if (!it || !it._all) throw new Error("Noticia no encontrada"); if (!API.uNoticias) { toast("Falta endpoint u_noticia.php en backend", "warning", 3500); return !1 } const body = { ...it._all, estatus: 1 }; await postJSON(API.uNoticias, body); return !0 }
  ...
  function renderNoticiaDrawer(dataset) {
    ...
    setTimeout(() => {
      try {
        const be = qs("#btn-edit"); if (be) be.addEventListener("click", e => { e.stopPropagation(); state.currentDrawer = { type: "noticia", id: n.id, mode: "edit" }; qs("#drawer-body").innerHTML = renderNoticiaDrawer({ id: String(n.id) }) });
        const bc = qs("#btn-cancel"); if (bc) bc.addEventListener("click", e => { e.stopPropagation(); state.currentDrawer = { type: "noticia", id: n.id, mode: "view" }; qs("#drawer-body").innerHTML = renderNoticiaDrawer({ id: String(n.id) }) });
        const bs = qs("#btn-save"); if (bs) bs.addEventListener("click", async e => { e.stopPropagation(); try { const payload = { ...n, ...readNoticiaForm(n) }; await postJSON(API.uNoticias, payload); toast("Cambios guardados", "exito"); state.currentDrawer = { type: "noticia", id: n.id, mode: "view" }; await loadNoticias(); const re = state.data.find(x => x.id === n.id); if (re) openDrawer("Noticia · " + re.titulo, renderNoticiaDrawer({ id: String(re.id) })) } catch (err) { gcLog(err); toast("Error al guardar", "error") } });
        ...
        const br = qs("#btn-reactivar"); if (br) br.addEventListener("click", async e => { e.stopPropagation(); const ok = await reactivateNoticia(n.id); if (ok) { toast("Noticia reactivada", "exito"); await loadNoticias(); const re = state.data.find(x => x.id === n.id); if (re) openDrawer("Noticia · " + re.titulo, renderNoticiaDrawer({ id: String(re.id) })) } });
  ...
  async function loadTutores() {
    ...
    const tt = qs(".tt-title"); if (tt) tt.textContent = "Tutores:"; const ttStatus = qs("#tt-status"); if (ttStatus) { ttStatus.textContent = "Activos e Inactivos"; ttStatus.classList.remove("badge-inactivo"); ttStatus.classList.add("badge-activo") }
    showSkeletons();
    try {
      const [act, inact] = await Promise.all([postJSON(API.tutores, { estatus: 1 }), postJSON(API.tutores, { estatus: 0 })]);
      const raw = [].concat(Array.isArray(act) ? act : [], Array.isArray(inact) ? inact : []);
      state.raw = raw; state.data = raw.map(t => ({ id: t.id, nombre: t.nombre, descripcion: t.descripcion, estatus: ntf(t.estatus), fecha: t.fecha_creacion, _all: t }));
      drawTutores()
  ...
  function renderTutorDrawer(dataset) {
    ...
    const mode = (state.currentDrawer && state.currentDrawer.mode) || (item ? "view" : "create");
    const isCreate = mode === "create" || !item, isEdit = mode === "edit", isView = mode === "view" && !!item;
    const t = isCreate ? { id: null, nombre: "", descripcion: "", estatus: 1, fecha_creacion: "", creado_por: Number((currentUser && currentUser.id) || 0) || 1 } : item ? item._all : null; if (!t) return "<p>No encontrado.</p>";
    ...
    else if (isAdminUser) { const isInactive = Number(t.estatus) === 0; controls = '<div class="gc-actions">' + (isView ? '<button class="gc-btn" id="btn-edit">Editar</button>' : "") + (isEdit ? '<button class="gc-btn gc-btn--ghost" id="btn-cancel">Cancelar</button>' : "") + (isEdit ? '<button class="gc-btn gc-btn--primary" id="btn-save">Guardar</button>' : "") + '<button class="gc-btn gc-btn--danger" id="btn-delete" data-step="1">Eliminar</button>' + (isInactive ? '<button class="gc-btn gc-btn--success" id="btn-reactivar">Reactivar</button>' : "") + "</div>" }
    let html = "" + controls
    ...
    setTimeout(() => {
      try {
        disableDrawerInputs(!(isEdit || isCreate));
        const btnSave = qs("#btn-save"); if (btnSave) btnSave.addEventListener("click", async e => { e.stopPropagation(); try { if (isCreate) await saveTutorCreate(); else await saveTutorUpdate(item) } catch (err) { gcLog(err); toast("Error al guardar", "error") } });

        const btnEdit = qs("#btn-edit"); if (btnEdit) btnEdit.addEventListener("click", e => { e.stopPropagation(); state.currentDrawer = { type: "tutor", id: item ? item.id : null, mode: "edit" }; qs("#drawer-body").innerHTML = renderTutorDrawer({ id: String(item ? item.id : "") }) });
        const btnCancel = qs("#btn-cancel"); if (btnCancel) btnCancel.addEventListener("click", e => { e.stopPropagation(); if (isCreate) { closeDrawer() } else { state.currentDrawer = { type: "tutor", id: item ? item.id : null, mode: "view" }; qs("#drawer-body").innerHTML = renderTutorDrawer({ id: String(item ? item.id : "") }) } });
  ...
  function drawCuentas() {
    const title = qs("#mod-title"); if (title) title.textContent = "Cuenta";
    const tt = qs(".tt-title"); if (tt) tt.textContent = "Cuentas:"; const ttStatus = qs("#tt-status"); if (ttStatus) { ttStatus.textContent = "Disponible"; ttStatus.classList.remove("badge-inactivo"); ttStatus.classList.add("badge-activo") }
    const desktopTable = qs(".recursos-table"); if (desktopTable) desktopTable.style.display = "none"; const mobileTable = qs(".recursos-table-mobile"); if (mobileTable) mobileTable.style.display = "none";
    const d = qs("#recursos-list"); if (d) d.innerHTML = ""; const m = qs("#recursos-list-mobile"); if (m) m.innerHTML = "";
    const host = qs(".recursos-box"); if (!host) return; let mount = document.getElementById("cuenta-menu"); if (!mount) { mount = document.createElement("div"); mount.id = "cuenta-menu"; host.appendChild(mount) }
    mount.innerHTML = `<div class="gc-card-grid" style="margin:12px 0 20px;">
...
<div class="gc-card"><img src="/ASSETS/admin/cuentaMenu/notificaciones.png" alt="" width="28" height="28"><div><div class="gc-card-title">Notificaciones / Preferencias</div><div class="gc-muted">Gestiona alertas dentro de la app, correos y push.</div></div><button id="btn-notifications" class="gc-btn gc-btn--primary gc-card-cta">Abrir</button></div>`;
    const pag1 = qs("#pagination-controls"); if (pag1) pag1.innerHTML = ""; const pag2 = qs("#pagination-mobile"); if (pag2) pag2.innerHTML = "";
    function safeOpen(fnName) { const fn = window[fnName]; if (typeof fn === "function") fn(); else toast("Modal no disponible aún", "warning") }
    const del = mount.querySelector("#btn-delete-account"); if (del) del.addEventListener("click", () => { safeOpen("openModalDeleteAccount") });
    const pr = mount.querySelector("#btn-privacy"); if (pr) pr.addEventListener("click", () => { safeOpen("openModalPrivacy") });
    const no = mount.querySelector("#btn-notifications"); if (no) no.addEventListener("click", () => { safeOpen("openModalNotifications") })
  }
