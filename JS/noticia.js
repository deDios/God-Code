...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!lista || !textarea || !contador || !btnCancelar || !btnEnviar) {
    console.warn("Comments script: missing required DOM elements");
    return;
  }
  ...
  textarea.addEventListener("keydown", e => {
    if ((e.key === "Enter" || e.key === " ") && !e.shiftKey && !btnEnviar.disabled) {
      e.preventDefault();
      btnEnviar.click();
    }
  });
  ...
  function crearComentarioHTML(data) {
    ...
    if (data.respuestas?.length) {
      ...
      cont.className = "subcomentarios subrespuestas";
  ...
  // -------- Manejo global clicks y keys --------
  document.addEventListener("click", handleComentarioAction);
  document.addEventListener("keydown", e => {
    // permitir Enter/Space en elementos con role button
  ...
  async function handleComentarioAction(e) {
    ...
    // LIKE/DISLIKE
    const react = e.target.closest(".reaccion");
    if (react) {
      e.preventDefault();
      if (!usuarioId) {
      ...
      const tipo = react.dataset.tipo;
      const cid = react.dataset.id;
      const comDiv = react.closest(".comentario, .subcomentario");
      ...
      try {
        ...
        if (url === endpointReaccion && json.mensaje.toLowerCase().includes("registrada")) {
          gcToast(`¡Gracias por tu ${tipo}!`, "exito");
        } else if (url === endpointQuitarReaccion && json.mensaje.toLowerCase().includes("eliminada")) {
          gcToast("Reacción eliminada.", "exito");
        } else {
          gcToast(json.mensaje || "Error al reaccionar", "warning");
        }
