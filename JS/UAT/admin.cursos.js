...
(() => {
  ...
  const S = {
    ...
    maps: { tutores:null, prioridad:null, categorias:null, calendario:null, tipoEval:null, actividades:null },
  ...
  function getCreatorId(){
    try{
      ...
      console.warn(TAG,"Cookie usuario sin id numérico:", u?.id); return null;
    }catch(e){ console.error(TAG,"getCreatorId() error:", e); return null; }
  }
  ...
  function withBust(u){
    ...
    catch{ const sep = u.includes("?") ? "&" : "?"; return u + sep + "v=" + Date.now(); }
  }

  /* ---------- Imágenes ---------- */
  function cursoImgUrl(id, ext="png"){ return `/ASSETS/cursos/img${Number(id)}.${ext}`; }
  function noImageSvgDataURI(){ const svg="<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 160 90'><rect width='100%' height='100%' fill='#f3f3f3'/><path d='M20 70 L60 35 L95 65 L120 50 L140 70' stroke='#c9c9c9' stroke-width='4' fill='none'/><circle cx='52' cy='30' r='8' fill='#c9c9c9'/></svg>"; return "data:image/svg+xml;utf8,"+encodeURIComponent(svg); }
  async function resolveCursoImg(id){
    const ok = url => new Promise(res=>{ const i=new Image(); i.onload=()=>res(true); i.onerror=()=>res(false); i.src=url; });
    const p = withBust(cursoImgUrl(id,"png")), j = withBust(cursoImgUrl(id,"jpg"));
    if (await ok(p)) return p; if (await ok(j)) return j; return noImageSvgDataURI();
  }
  ...
  /* ---------- Map helpers ---------- */
  function arrToMap(arr){ const m={}; (Array.isArray(arr)?arr:[]).forEach(x=>{ m[x.id]=x.nombre||x.titulo||"#"+x.id; }); m._ts=Date.now(); return m; }
  function mapToOptions(map, sel){ return Object.keys(map||{}).filter(k=>k!=="_ts").map(id=>`<option value="${id}"${+id===+sel?" selected":""}>${esc(map[id])}</option>`).join(""); }
  function mapLabel(map, id){ if(!map) return "-"; const k=String(id??""); return k in map ? (map[k] ?? "-") : "-"; }

  // export utilidades (por si otros módulos las usan)
  window.gcUtils = Object.assign({}, window.gcUtils||{}, {
    qs,qsa,esc,fmtMoney,fmtBool,fmtDate,STATUS_LABEL,ORDER_CURSOS,
    postJSON,withBust,toast,cursoImgUrl,resolveCursoImg,noImageSvgDataURI,
  ...
  function renderCursos(){
    ...
    if (hostD){
      if (pageRows.length===0){
        ...
        hostD.insertAdjacentHTML("beforeend", pageRows.map(rebuildRowHTML).join(""));
      }
    }

    // Mobile (solo título + botón ver)
    if (hostM){
      if (pageRows.length===0){
        hostM.insertAdjacentHTML("beforeend", `<div class="table-row"><div class="col-nombre">Sin resultados</div></div>`);
      }else{
        hostM.insertAdjacentHTML("beforeend", pageRows.map(it=>`
  ...
  function renderPagination(total){
    ...
    [qs("#pagination-controls"), qs("#pagination-mobile")].forEach(cont=>{
      ...
      const mk=(txt,dis,cb,cls="page-btn")=>{ const b=document.createElement("button"); b.textContent=txt; b.className=cls+(dis?" disabled":""); if(dis) b.disabled=true; else b.onclick=cb; return b; };
      cont.appendChild(mk("‹", S.page===1, ()=>{ S.page=Math.max(1,S.page-1); renderCursos(); }, "arrow-btn"));
      for(let p=1;p<=totalPages && p<=7;p++){ const b=mk(String(p), false, ()=>{ S.page=p; renderCursos(); }); if (p===S.page) b.classList.add("active"); cont.appendChild(b); }
      cont.appendChild(mk("›", S.page===totalPages, ()=>{ S.page=Math.min(totalPages,S.page+1); renderCursos(); }, "arrow-btn"));
    });
  }

  // Delegación para filas (bind una vez)
  const desktopBody = qs("#recursos-list");
  if (desktopBody && !desktopBody._b){
    desktopBody._b = true;
    desktopBody.addEventListener("click", (e)=>{
      const row = e.target.closest(".table-row[data-id]");
      if (!row) return;
      const id = Number(row.dataset.id||0);
      if (id) openCursoView(id);
    });
  }
  const mobileBody = qs("#recursos-list-mobile");
  if (mobileBody && !mobileBody._b){
    mobileBody._b = true;
  ...
  function paintActions(it){
    const actions = qs("#curso-actions-view");
    if (!actions) return;
    // Construimos SIEMPRE los dos botones (edit + toggle) y delegamos eventos en el contenedor
    const isInactive = +it.estatus === 0;
    const toggleLabel = isInactive ? "Reactivar" : "Eliminar";
    const toggleClass = isInactive ? "gc-btn gc-btn--success" : "gc-btn gc-btn--danger";
    // Nota: para confirmación usaremos data-confirm internamente
    actions.innerHTML = `
      <button class="gc-btn" id="btn-edit" data-action="edit">Editar</button>
      <button class="${toggleClass}" id="btn-toggle" data-action="toggle-status">${toggleLabel}</button>
    `;
    actions.dataset.estatus = String(it.estatus);
  }

  // Delegación de eventos del área de acciones (bind una vez)
  const actionsHost = qs("#curso-actions-view");
  if (actionsHost && !actionsHost._b){
    actionsHost._b = true;
    let confirmTimer = null;

    actionsHost.addEventListener("click", async (e)=>{
      const btn = e.target.closest("button");
      if (!btn) return;
      const action = btn.dataset.action;

      // Seguridad
      if (!S.current?.id){ toast("No hay curso seleccionado.", "error"); return; }
      ...
      if (action === "toggle-status"){
        ...
        // Si vamos a eliminar (soft delete) pide confirmación
        if (!isInactive){
          if (btn.dataset.confirm !== "1"){
            // Primera pulsación: cambia a warning/¿Confirmar?
            btn.dataset.confirm = "1";
            btn.textContent = "¿Confirmar?";
            btn.className = "gc-btn gc-btn--warning";
            if (confirmTimer) clearTimeout(confirmTimer);
            confirmTimer = setTimeout(()=>{
              btn.dataset.confirm = "0";
              const _isInactive = +S.current._all.estatus === 0;
              btn.textContent = _isInactive ? "Reactivar" : "Eliminar";
              btn.className   = _isInactive ? "gc-btn gc-btn--success" : "gc-btn gc-btn--danger";
            }, 2500);
            return;
          }
        }
        ...
        try{
          if (isInactive){
            ...
            if (!ok){ it.estatus = prev; updateRowStatusCell(it.id, it.estatus); paintActions(it); }
            else { toast("Curso reactivado", "exito"); }
          }else{
            ...
            if (!ok){ it.estatus = prev; updateRowStatusCell(it.id, it.estatus); paintActions(it); }
            else { toast("Curso movido a Inactivo", "exito"); }
          }
        } finally {
          btn.disabled = false;
          // Limpia confirmación si existía
          btn.dataset.confirm = "0";
  ...
  function updateRowStatusCell(id, estatus){
    // Actualiza fuente de verdad S.data
    const idx = S.data.findIndex(x=> +x.id === +id);
    if (idx>=0){ S.data[idx].estatus = estatus; }

    // Actualiza celda en tabla desktop
    const row = qs(`.table-row[data-id="${id}"]`);
    if (row){
      const cell = row.querySelector(".col-status");
      if (cell) cell.textContent = STATUS_LABEL[estatus] || estatus;
    }
    // (En mobile no mostramos status, así que no hay nada que tocar)
    // Si quieres resort local por estatus, puedes hacerlo aquí (opcional).
  }
  ...
  async function fillCursoView(it){
    ...
    const title = qs("#drawer-curso-title"); if (title) title.textContent = "Curso · " + (it.nombre || "—");
    ...
    // Pinta acciones con estilos reutilizados
    paintActions(it);
  }
  ...
  async function uploadCursoCover(cursoId, file){
    ...
    const text = await res.text().catch(()=> ""); console.log(TAG,"upload HTTP", res.status, "raw:", text);
  ...
  function previewOverlay(file, { onConfirm, onCancel }){
    ...
    const o = document.createElement("div"); o.style.cssText="position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;background:rgba(17,24,39,.55)";
    const m = document.createElement("div"); m.style.cssText="background:#fff;border-radius:14px;box-shadow:0 20px 40px rgba(0,0,0,.25);width:min(920px,94vw);max-height:90vh;overflow:hidden;display:flex;flex-direction:column;";
    m.innerHTML = `
  ...
  function mountCursoMediaEdit(containerEl, cursoId){
        ...
        <div class="media-card">
          <figure class="media-thumb">
            ...
            <button class="icon-btn media-edit" type="button" title="Editar imagen" aria-label="Editar imagen">
              <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" fill="currentColor"></path></svg>
            </button>
          </figure>
          <div class="media-meta"><div class="media-label">Portada</div></div>
        </div>
    ...
    if (!pencil || pencil._b) return; pencil._b = true;
  ...
  async function updateCursoStatus(id, estatus, extra={}){
    ...
    try{
      ...
      return true;
    }catch(err){ console.error(TAG,"updateCursoStatus ERROR:", err); toast("No se pudo actualizar el estatus.","error"); return false; }
  }
  ...
  async function saveCurso(){
    ...
    try{
      ...
      if (body.id == null){
        ...
        if (creado_por == null){ toast("No se puede crear: falta el id de usuario (creado_por).","error"); return; }
        ...
        if (newId && S.tempNewCourseImage instanceof File){
          try{
            ...
            const imgView = document.querySelector("#curso-cover-view"); if (imgView) imgView.src = withBust(url);
            toast("Imagen subida","exito");
          }catch(e){ console.error(TAG,"Upload diferido falló:", e); toast("Curso creado, pero la imagen no se pudo subir.","error"); }
        }
        ...
        // Actualiza el S.data local y la fila
        const idx = S.data.findIndex(x=> +x.id===+body.id);
        if (idx>=0){
          S.data[idx] = { ...S.data[idx], ...body };
          updateRowStatusCell(body.id, S.data[idx].estatus);
        }
        if (S.current){ S.current._all = { ...S.current._all, ...body }; }
      }

      toast("Curso guardado","exito");

      if (newId){
        // refresca lista mínima: trae el curso recién insertado (opcional) o recarga suave
        await loadCursos();
        const it = (S.data||[]).find(x=> +x.id === +newId) || S.data[0];
        if (it){ S.current = { id: it.id, _all: it }; setDrawerMode("view"); await fillCursoView(it); }
      } else if (S.current?.id){
        // Para update, solo reabrimos el mismo
        const it = (S.data||[]).find(x=> +x.id === +S.current.id);
        if (it){ S.current._all = it; setDrawerMode("view"); await fillCursoView(it); }
      }
  ...
  async function mount(){
    ...
    try{
      ...
      if (s && !s._b){
        s._b = true;
        s.addEventListener("input", (e)=>{ S.search = e.target.value||""; S.page=1; renderCursos(); });
      }
  ...
  function openCursoCreate(){
    ...
    const blank = {
      ...
      dirigido:"", competencias:"", tutor:null, horas:null, precio:0, estatus:1, fecha_inicio:"",
      prioridad:null, categoria:null, calendario:null, tipo_evaluacion:null, actividades:null, certificado:0
    };
    S.current = { id:null, _all: blank };
    openDrawerCurso(); setDrawerMode("edit"); fillCursoEdit(blank);
  }

  // expone API pública para el router
  window.cursos = { mount, openCreate: openCursoCreate };
  window.cursosLoad = { loadCatalogos, loadCursos }; // opcional/legacy
