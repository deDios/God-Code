...
(() => {
  ...
  if (overlay && !overlay._b){ overlay._b=true; overlay.addEventListener("click", closeDrawerCurso); }
  ...
  function mountCursoMediaEdit(containerEl, cursoId){
    ...
    (async () => { img.src = await resolveCursoImg(cursoId || 0); })();
    img.onerror = () => { img.onerror=null; img.src = noImageSvgDataURI(); };

    // si no hay ID (modo crear), bloquear subida hasta guardar
    if (!cursoId) {
      pencil.disabled = true;
      pencil.title = "Guarda el curso primero para subir imagen";
      pencil.classList.add("disabled");
      return;
    }
  ...
  async function saveCurso(){
    ...
    try {
      ...
      if (!it) it = (S.data || []).find(x => x.nombre === body.nombre) || S.data?.[0];
      ...
      if (it) {
        ...
        // si no se encontró, permanece en edición
        setDrawerMode("edit");
      }
    } catch (err) {
      console.error(err);
  ...
  function openCursoCreate(){
    const blank = {
      ...
      dirigido: "", competencias: "",
      tutor: null, horas: null, precio: 0,
      estatus: 1, fecha_inicio: "",
      prioridad: null, categoria: null, calendario: null, tipo_evaluacion: null, actividades: null,
      certificado: 0
    };
