...
(() => {
  ...
  function renderDrawer(dataset) {
    ...
    setTimeout(async () => {
      ...
      if (pencil && !pencil._b) {
        ...
        pencil.addEventListener("click", () => {
          ...
          input.addEventListener("change", async () => {
            ...
            if (file.size > MAX_UPLOAD_BYTES) return toast("La imagen excede 10MB.", "error");
            ...
            openPreview({
              ...
              confirm: true,
              onConfirm: async () => {
                const tid = Number((state.currentDrawer?.id) || 0);
                const imgEdit = document.querySelector("#tutor-img-edit");
                const info = document.querySelector("#tutor-img-info");
                ...
                try {
                  ...
                  setImgWithFallback(
                    imgEdit,
                    `/ASSETS/tutor/tutor_${tid}.png?v=${Date.now()}`,
                    `/ASSETS/tutor/tutor_${tid}.jpg?v=${Date.now()}`,
                    noImageSvgDataURI()
                  );
                  if (info) info.textContent = "Imagen actualizada";
                  toast("Imagen actualizada", "exito");
                } catch (err) {
                  console.error("[Tutores] upload img error:", err);
      ...
      const imgView = document.querySelector("#tutor-img-view");
if (imgView) {
  const openPreview = getOpenImagePreview();
  imgView.style.cursor = "zoom-in";
  imgView.addEventListener("click", () => openPreview({ src: imgView.src, title: "Vista previa · Foto del tutor", confirm: false }));
}
