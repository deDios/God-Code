...
(() => {
  ...
  (() => {
    ...
    // ---- Preview local con confirm (idéntico a Noticias) ----
    function openImagePreviewLocal({ src, file, title = "Vista previa", confirm = false, onConfirm }) {
      ...
      head.innerHTML = `<div style="font-weight:700;font-size:1.05rem;">${String(title)}</div>
    ...
    <div style="font-size:.92rem;color:#444;line-height:1.35;">
      ${file ? `
        ...
        <div><strong>Tipo:</strong> ${file.type || "-"}</div>` : `<div style="color:#666;">Solo lectura</div>`}
    ...
    // Forzar modal local si hay confirm (evita /VIEW/UAT/undefined)
    ...
    async function pickImageAndPreview({ title = "Vista previa", accept = "image/png,image/jpeg", maxMB = MAX_UPLOAD_MB, onConfirm }) {
        ...
        input.addEventListener("change", async () => {
          ...
          openPreview({
            ...
            onConfirm: async () => {
              ...
              catch (e) { console.error("[pickImageAndPreview] onConfirm error:", e); toast("No se pudo completar la carga.", "error"); resolve(null); }
            }
          });
        });
    ...
    function draw() {
      ...
      if (cnt)
        cnt.textContent = `${filtered.length} ${filtered.length === 1 ? "elemento" : "elementos"
          }`;
    ...
    `;
      ...
      setTimeout(async () => {
        ...
        if (isCreate) {
          ...
          titleEl &&
            (titleEl.textContent = `Tutor · ${item ? item.nombre : ""
              } (edición)`);
          state.currentDrawer = {
            type: "tutor",
            id: item?.id || null,
        ...
        if (window.gcBindCharCounters) {
          window.gcBindCharCounters(
            document.getElementById("drawer-tutor-body")
          );
        }
      }, 0);
  ...
  function openImagePreviewSafe(opts) {
    const fn =
      (window.gcPreview && window.gcPreview.openImagePreview) ||
      window.openImagePreview;
    if (typeof fn === "function") {
      fn(opts);
      return;
    }
    const src = opts?.src || (opts?.file ? URL.createObjectURL(opts.file) : "");
  ...
  function bindImagePreview(el, { title = "Vista previa" } = {}) {
    ...
    el.addEventListener("click", (e) => {
      ...
      if (!src) return;
      openImagePreviewSafe({ src, confirm: false, title });
    });
  }
