...
/**
 * CURSOS (UAT) — Ajustado al DDL de gc_cursos
 * - Lista y búsqueda
 * - Drawer crear/editar
 * - Payload FormData mapeado al DDL (con derivados/defaults seguros)
 * - Subida de imagen con clave 'imagen'
 */
export const Cursos = {
  async mount(){
    gcLog('Cursos.mount');
  ...
  bindGlobalHandlers(){
    ...
    root.addEventListener('click', async (e)=>{
      ...
      if (!id) return;
      // intentar pedir el curso por id usando la misma API de list con q
      let row = null;
      try {
...
export function buildCursoPayloadDDL(formEl, userId){
  ...
  fd.append('fecha', fecha_inicio); // alias por compat (i/u viejos)
