    ...
    try {
      ...
      if ($list) {
        ...
        tutores.forEach((t) => {
          ...
          row.className = 'table-row'; // <- importante para tu CSS

          row.innerHTML = `
      <div class="col-nombre txt-ellipsis" title="${esc(t.nombre ?? '')}">${esc(t.nombre ?? '-')}</div>
      <div class="col-fecha">${esc(t.fecha_creacion ?? '-')}</div>
      <div class="col-status">${esc(({ "1": "Activo", "0": "Inactivo" }[String(t.estatus)] || t.estatus))}</div>
      <div class="col-acc">
    ...
    `;
      ...
      if ($listMobile) {
        ...
        tutores.forEach((t) => {
          ...
          card.className = 'table-row'; // o 'gc-card' si tu CSS móvil lo usa
          card.innerHTML = `
      <div class="col-nombre">${esc(t.nombre ?? '-')}</div>
      <div class="col-fecha">${esc(t.fecha_creacion ?? '-')}</div>
      <div class="col-status">${esc(({ "1": "Activo", "0": "Inactivo" }[String(t.estatus)] || t.estatus))}</div>
      <div class="col-acc"><button class="gc-btn mini ver">Ver</button></div>
    `;
          card.querySelector('.ver')?.addEventListener('click', () => openTutorViewByObj(t));
