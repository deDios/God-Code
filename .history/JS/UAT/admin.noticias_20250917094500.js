...
(() => {
  ...
  const NAPI = {
    ...
    autores: API_BASE + "c_usuarios.php",
    categorias: API_BASE + "c_categorias_noticia.php"
  };
  const NUPLOAD = { portada: API_BASE + "u_noticiaImgenes.php" };
  ...
  // ===================== Listado / Init: Noticias =====================
  async function noticiasInit() {
    try {
      // 1) Opcional: cualquier carga previa (catálogos, estados, etc.)
      if (typeof loadCatNoticias === 'function') {
        await loadCatNoticias();
      }

      // 2) Obtener data (usa NAPI si existe; si no, fallback directo al endpoint)
      let noticias = [];
      if (typeof NAPI === 'object') {
        if (typeof NAPI.list === 'function') {
          noticias = await NAPI.list();
        } else if (typeof NAPI.noticias === 'function') {
          noticias = await NAPI.noticias();
        }
      }
      if (!Array.isArray(noticias)) noticias = [];

      // 3) Render “tabla” desktop
      ...
      if ($list) {
        ...
        noticias.forEach((n) => {
          ...
          tr.className = 'gc-row';
          tr.innerHTML = `
          <td class="txt-ellipsis" title="${n.titulo ?? ''}">${n.titulo ?? '-'}</td>
          <td>${n.estatus ?? '-'}</td>
        ...
        `;
          // abrir en modo ver/editar
          tr.querySelector('.ver')?.addEventListener('click', () => {
            if (typeof openNoticiaViewByObj === 'function') {
              openNoticiaViewByObj(n);
            } else if (typeof openDrawerNoticia === 'function' && typeof fillNoticiaView === 'function') {
              openDrawerNoticia('view'); // o 'edit' según tu UX
              fillNoticiaView(n);
            } else {
              console.warn('[Noticias] No hay viewer expuesto. Abriendo crear como fallback.');
              if (typeof openNoticiaCreate === 'function') openNoticiaCreate();
            }
          });
          $list.appendChild(tr);
        });
      ...
      // 4) Render “lista” mobile (si usas contenedor aparte)
      ...
      if ($listMobile) {
        ...
        noticias.forEach((n) => {
          ...
          li.innerHTML = `
          <div class="gc-card-title">${n.titulo ?? '-'}</div>
          <div class="gc-card-sub">${n.fecha_creacion ?? '-'}</div>
          <div class="gc-badges"><span class="gc-chip">${n.estatus ?? '-'}</span></div>
