    ...
    try {
      ...
      if ($list) {
        ...
        noticias.forEach((n) => {
          ...
          tr.className = "gc-row";
          tr.innerHTML = `
            <td class="txt-ellipsis" title="${esc(n.titulo ?? "")}">${esc(n.titulo ?? "-")}</td>
            <td>${esc(estatusLabel(n.estatus))}</td>
            <td>${esc(n.fecha_creacion ?? "-")}</td>
            <td><button class="gc-btn mini ver">Ver</button></td>
          `;
          tr.querySelector(".ver")?.addEventListener("click", () => {
            openDrawerNoticia();
            setNoticiaMode("view");
            NS.current = { id: n.id, _all: n };
            fillNoticiaView(n);
          });
          $list.appendChild(tr);
        });
      ...
      if ($listMobile) {
        ...
        noticias.forEach((n) => {
          ...
          li.innerHTML = `
            <div class="gc-card-title">${esc(n.titulo ?? "-")}</div>
            <div class="gc-card-sub">${esc(n.fecha_creacion ?? "-")}</div>
            <div class="gc-badges"><span class="gc-chip">${esc(estatusLabel(n.estatus))}</span></div>
