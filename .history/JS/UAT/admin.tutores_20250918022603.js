(() => {
  ...
  const APIx = (function getAPI(){
    ...
    // Usa tus endpoints reales si ya están en window.API; si no, defaults locales
    return {
  ...
  const gcToast = (window.gcToast) || ((m)=>{ _gcLog("gcToast (fallback):", m); alert(m); });
  ...
  function openTutorDrawer(title, html) {
    ...
    if (!aside) {
      _gcLog("openTutorDrawer: no existe #drawer-tutor, fallback openDrawer()");
  ...
  async function mount() {
    ...
    if (s) {
      s.oninput = () => { S.search = s.value || ""; _gcLog("search:change", S.search); draw(); };
    }
  ...
  function renderDrawer(dataset) {
    ...
    setTimeout(() => {
      ...
      _gcLog("drawer:mode", state.currentDrawer);
      ...
      if (isCreate) {
        ...
        if (cont && tid) {
          _gcLog("media:mount", { id: tid, editable: Boolean(isEdit && isAdminUser) });
          mountReadOnlyMedia({
            container: cont,
            type: "tutor",
            id: tid,
  ...
  async function renderTutorCursosChips(tutorId, containerSel = "#tutor-cursos") {
    ...
    try {
      ...
      const statusClass = v => {
        ...
        if (n === 2) return "s-2";
        if (n === 3) return "s-3";
        if (n === 4) return "s-4"; // define CSS .s-4 si quieres azul
        return "s-0"; // 0 o 5
      };
      ...
      host.querySelectorAll(".curso-chip").forEach(btn => {
        btn.addEventListener("click", (e) => {
          ...
          if (typeof window.openDrawer === "function") {
            ...
            // Fallback (si usas drawer separado para curso)
