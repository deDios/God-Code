...
(() => {
  ...
  const { qs, qsa, esc, fmtDate, postJSON, toast, withBust } = window.gcUtils;
  ...
  const TAPI = {
    ...
    // inserts/updates
    iTutor: API_BASE + "i_tutor.php",
    uTutor: API_BASE + "u_tutor.php",
  };
  const TUPLOAD = { foto: API_BASE + "u_tutorImg.php" };
  ...
  async function fillTutorView(t) {
    ...
    put("#tv_correo", t.correo);
    put("#tv_telefono", t.telefono);
    put("#tv_estatus", ({ "1": "Activo", "0": "Inactivo" }[String(t.estatus)] || t.estatus));
    put("#tv_resumen", t.resumen);
  ...
  function fillTutorEdit(t) {
    ...
    setVal("t_correo", t.correo);
    setVal("t_telefono", t.telefono);
    setVal("t_resumen", t.resumen);
  ...
  async function saveTutor() {
    const body = {
      ...
      nombre: val("t_nombre"),
      correo: val("t_correo"),
      telefono: val("t_telefono"),
      resumen: val("t_resumen"),
      descripcion: val("t_descripcion"),
      estatus: Number(val("t_estatus") || 1)
    };
    if (!body.nombre || !body.correo) {
      toast("Completa al menos nombre y correo.", "error"); return;
    }
    try {
      if (body.id == null) await postJSON(TAPI.iTutor, body);
      else await postJSON(TAPI.uTutor, body);

      toast("Tutor guardado", "exito");
  ...
  async function tutoresInit() {
    try {
      // 1) Cargas previas si aplica (catálogos, etc.)
      if (typeof loadCatTutores === 'function') {
        await loadCatTutores();
      }

      // 2) Obtener data
      let tutores = [];
      if (typeof TAPI === 'object') {
        if (typeof TAPI.list === 'function') {
          tutores = await TAPI.list();
        } else if (typeof TAPI.tutores === 'function') {
          tutores = await TAPI.tutores();
        }
      }
      ...
      if ($list) {
        ...
        tutores.forEach((t) => {
          ...
          tr.className = 'gc-row';
          tr.innerHTML = `
          <td class="txt-ellipsis" title="${t.nombre ?? ''}">${t.nombre ?? '-'}</td>
          <td>${t.correo ?? '-'}</td>
          <td>${t.telefono ?? '-'}</td>
          <td>
            <button class="gc-btn mini ver">Ver</button>
          </td>
        `;
          tr.querySelector('.ver')?.addEventListener('click', () => {
            if (typeof openTutorViewByObj === 'function') {
              openTutorViewByObj(t);
            } else if (typeof openTutorDrawer === 'function' && typeof fillTutorView === 'function') {
              openTutorDrawer('view');
              fillTutorView(t);
            } else {
              console.warn('[Tutores] No hay viewer expuesto. Abriendo crear como fallback.');
              if (typeof openTutorCreate === 'function') openTutorCreate();
            }
          });
          $list.appendChild(tr);
        });
      ...
      if ($listMobile) {
        ...
        tutores.forEach((t) => {
          ...
          li.innerHTML = `
          <div class="gc-card-title">${t.nombre ?? '-'}</div>
          <div class="gc-card-sub">${t.correo ?? '-'}</div>
          <div class="gc-card-sub">${t.telefono ?? '-'}</div>
