(() => {
  ...
  const API = (function getAPI(){
    ...
    return {
      ...
      cCursos  : A.cursos    || A.cCursos || "/db/web/c_cursos.php",
  ...
  const noImageSvg = (window.noImageSvg) || (() => `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 120 120'><rect width='120' height='120' fill='%23eee'/><text x='50%' y='54%' text-anchor='middle' font-size='10' fill='%23888'>sin imagen</text></svg>`);
  ...
  function openTutorDrawer(title, html) {
    ...
    if (closeBtn && !closeBtn._bound) {
      ...
      _gcLog("openTutorDrawer: bound close");
    }
    const onEsc = (e) => { if (e.key === "Escape") { _gcLog("esc:close"); closeTutorDrawer(); document.removeEventListener("keydown", onEsc); } };
    document.addEventListener("keydown", onEsc);
    overlay?.addEventListener("click", closeTutorDrawer, { once: true });
  ...
  function closeTutorDrawer() {
    ...
    if (aside) {
      try {
        ...
        if (ae && aside.contains(ae)) ae.blur(); // Fix ARIA foco oculto
      } catch {}
  ...
  function renderDrawer(dataset) {
    ...
    if (!t) { _gcLog("renderDrawer:not-found"); return "<p>No encontrado.</p>"; }
          ...
          <div id="create-media-tutor" class="media-grid">
            <div class="media-card">
              <figure class="media-thumb">
                ...
                <button class="gc-btn gc-btn--ghost" id="create-media-edit" style="margin-top:.5rem;">Cambiar imagen</button>
              </figure>
    ...
    const editImgBlock = `
      <div class="field">
        <label>Imagen</label>
        <div class="value">
          <div id="media-tutor-edit" class="media-grid" data-id="${t.id || (item ? item.id : "")}">
            <div class="media-card">
              <figure class="media-thumb">
                <img id="edit-media-thumb" alt="Foto" src="" />
                <div class="media-meta"><div class="media-label">Foto</div></div>
              </figure>
              <div style="display:flex; gap:.5rem; margin-top:.5rem;">
                <button class="gc-btn gc-btn--ghost" id="btn-change-img">Cambiar imagen</button>
                <button class="gc-btn gc-btn--primary" id="btn-upload-img" disabled>Subir</button>
              </div>
              <div class="media-help gc-soft">Formatos: JPG/PNG · Máx 2MB.</div>
            </div>
          </div>
        </div>
      </div>`;
    ...
    const editHTML = `
      <section id="tutor-edit" class="mode-edit"${(isCreate || isEdit) ? "" : " hidden"}>
        ...
        <!-- Acciones ABAJO como en otros drawers -->
    ...
    setTimeout(() => {
      ...
      qs("#btn-delete-tutor")?.addEventListener("click", async (e) => {
        ...
        if (step === "1") {
          ...
          setTimeout(() => {
            if (btn.getAttribute("data-step") === "2") { btn.textContent = "Eliminar"; btn.setAttribute("data-step","1"); }
          }, 4000);
          return;
        }
        try {
      ...
      // Media (vista / edición / crear)
      if (isCreate) {
        const card = qs("#create-media-tutor"), btn = qs("#create-media-edit"), thumb = qs("#create-media-thumb");
        if (btn && thumb && card) {
          btn.addEventListener("click", () => pickImageFile({
            onSelected: (file, blobUrl) => {
        ...
        // Cargar imagen actual y permitir cambio
        const tid = Number(t.id || item?.id || 0);
        const thumb = qs("#edit-media-thumb");
        const url = (mediaUrlsByType && mediaUrlsByType("tutor", tid)[0]) || `/ASSETS/tutor/tutor_${tid}.png`;
        if (thumb) {
          thumb.src = withBust(url);
          thumb.onerror = () => { thumb.onerror = null; thumb.src = 'data:image/svg+xml;utf8,' + encodeURIComponent(noImageSvg()); };
          _gcLog("media:tutor:edit:current", { id: tid, url });
        }
        const btnChange = qs("#btn-change-img");
        const btnUpload = qs("#btn-upload-img");
        let pendingFile = null, pendingBlobUrl = null;

        btnChange?.addEventListener("click", () => pickImageFile({
          onSelected: (file, blobUrl) => {
            pendingFile = file;
            pendingBlobUrl = blobUrl;
            if (thumb) thumb.src = blobUrl;
            btnUpload && (btnUpload.disabled = false);
            _gcLog("media:edit:selected", { size: file.size, type: file.type });
          }
        }));
        ...
        btnUpload?.addEventListener("click", async () => {
          ...
          try {
            _gcLog("media:edit:upload:start", { id: tid, size: pendingFile.size });
            await uploadTutorImagen(tid, pendingFile);
            gcToast("Imagen actualizada", "exito");
            pendingFile = null;
            if (pendingBlobUrl) { try { URL.revokeObjectURL(pendingBlobUrl); } catch {} pendingBlobUrl = null; }
            await reloadDrawerView(tid);
          } catch (err) {
            _gcLog("media:edit:upload:error", err);
            gcToast("No se pudo subir la imagen", "error");
          }
        });
      } else {
        // Modo vista: render helper + fallback si no inyecta <img>
        const tid = Number(t.id || item?.id || 0);
        const cont = qs("#media-tutor");
        if (cont && tid) {
          mountReadOnlyMedia({ container: cont, type: "tutor", id: tid, labels: ["Foto"], editable: false });
          // fallback si el helper no insertó img
          setTimeout(() => {
            if (!cont.querySelector("img")) {
              const url = (mediaUrlsByType && mediaUrlsByType("tutor", tid)[0]) || `/ASSETS/tutor/tutor_${tid}.png`;
              _gcLog("media:tutor:view:fallback", { id: tid, url });
              cont.innerHTML = `
  ...
  async function uploadTutorImagen(tutorId, file) {
    ...
    _gcLog("uploadTutorImagen:start", { tutorId, size: file?.size, type: file?.type, url });
    ...
    try { const json = JSON.parse(text); _gcLog("uploadTutorImagen:ok", json); return json; }
    catch { _gcLog("uploadTutorImagen:parse", text?.slice(0, 160)); return { _raw: text }; }
  }
  ...
  async function renderTutorCursosChips(tutorId, containerSel = "#tutor-cursos") {
    ...
    try {
      const all = await fetchAllCursosAnyStatusLocal();
      _gcLog("chips:data:allCursos", all?.length);
      const list = (all || []).filter(c =>
        Number(c.tutor) === Number(tutorId) || Number(c.id_tutor) === Number(tutorId)
      );
      ...
      // Orden: 1 Activo → 4 En curso → 2 Pausado → 3 Terminado → 0/5 Inactivo/Cancelado
      ...
      const statusClass = v => {
        ...
        if (n === 2) return "s-2";
        if (n === 3) return "s-3";
        if (n === 4) return "s-4"; // si no tienes CSS, cámbialo a s-3
        return "s-0";
      };

      const html = list.map(curso => {
        const img = (window.mediaUrlsByType && mediaUrlsByType("curso", curso.id)[0]) || "/ASSETS/cursos/img0.png";
      ...
      host.querySelectorAll(".curso-chip").forEach(btn => {
        btn.addEventListener("click", (e) => {
          ...
          if (typeof window.openDrawer === "function") {
            window.openDrawer("Curso · —", (window.renderCursoDrawer ? window.renderCursoDrawer({ id: String(cid) }) : "<p>Drawer Curso no disponible</p>"));
          } else {
            ...
            if (body) body.innerHTML = (window.renderCursoDrawer ? window.renderCursoDrawer({ id: String(cid) }) : "<p>Drawer Curso no disponible</p>");
          }
        });
      });
  ...
  // Llama c_cursos.php para distintos estatus y concatena (si no hay helper global)
  async function fetchAllCursosAnyStatusLocal() {
    if (typeof window.fetchAllCursosAnyStatus === "function") {
      _gcLog("fetchCursos:using-global-helper");
      return await window.fetchAllCursosAnyStatus();
    }
    _gcLog("fetchCursos:local:start", API.cCursos);
    const statuses = [1,4,2,3,0,5];
    const chunks = await Promise.all(statuses.map(st => postJSON(API.cCursos, { estatus: st }).catch(() => [])));
    const all = chunks.flat().filter(Boolean);
    _gcLog("fetchCursos:local:done", { total: all.length });
    return all;
  }
  ...
  function pickImageFile({ onSelected }) {
    ...
    input.addEventListener("change", () => {
      ...
      onSelected && onSelected(file, blobUrl);
    });
    input.click();
  }
  ...
  async function reloadDrawerView(id) {
    await load();
    const re = S.data.find(x => x.id === Number(id));
    if (re) {
      state.currentDrawer = { type: "tutor", id: Number(id), mode: "view" };
      openTutorDrawer("Tutor · " + re.nombre, renderDrawer({ id: String(re.id) }));
    }
  }
