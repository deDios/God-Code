...
(() => {
  ...
  async function resolveNoticiaImg(id, pos = 1) {
    ...
    return (await imgExists(url)) ? url : noImageSvgDataURI();
  }
  ...
  /* ---------- Helpers SCOPED al drawer de Noticias (evita colisiones con Cursos) ---------- */
  function nRoot() {
    return document.querySelector("#drawer-noticia");
  }
  function nqs(sel) {
    return nRoot()?.querySelector(sel) || null;
  }
  function nsetVal(id, v) {
    const el = nqs("#" + id);
  ...
  function nPutStatus(id, sel) {
    ...
    el.innerHTML = opts
      .map(o => `<option value="${o.v}"${+o.v===+sel ? " selected" : ""}>${o.l}</option>`)
      .join("");
  }
  function nPutText(sel, text) {
    const el = nqs(sel);
    if (!el) return;
  ...
  function setNoticiaDrawerMode(mode) {
    ...
    const v = nqs("#noticia-view"),
      e = nqs("#noticia-edit"),
      act = nqs("#noticia-actions-view");
    if (mode === "view") {
      v && (v.hidden = false);
      e && (e.hidden = true);
  ...
  async function loadNoticias() {
    ...
    S.data = flat;
    S.page = 1;
    console.log(TAG, "Noticias cargadas:", S.data.length, S.data);
    renderNoticias();
  }
  ...
  function paintNoticiaActions(it) {
    const cont = nqs("#noticia-actions-view");
    if (!cont) return;
  ...
  async function fillNoticiaView(n) {
    ...
    const title = nqs("#drawer-noticia-title");
    if (title) title.textContent = "Noticia · " + (n.titulo || "—");

    // Campos (modo vista)
    nPutText("#nv_titulo", n.titulo ?? "—");
    nPutText("#nv_desc_uno", n.desc_uno ?? "—");
    nPutText("#nv_desc_dos", n.desc_dos ?? "—");
    nPutText("#nv_estatus", STATUS_LABEL[n.estatus] || n.estatus);
    nPutText("#nv_creado_por", n.creado_por ?? "—");
    nPutText("#nv_fecha_creacion", n.fecha_creacion ?? "—");

    await mountNoticiaMediaView(nqs("#media-noticia"), n.id);

    const pre = nqs("#json-noticia");
  ...
  function mountNoticiaMediaEdit(containerEl, noticiaId) {
    ...
    if (noticiaId) {
      ...
      // modo crear: placeholder y buffers
  ...
  function fillNoticiaEdit(n) {
    ...
    nsetVal("f_titulo", n.titulo);
    nsetVal("f_desc_uno", n.desc_uno);
    nsetVal("f_desc_dos", n.desc_dos);
    nPutStatus("f_estatus", n.estatus);

    mountNoticiaMediaEdit(nqs("#media-noticia-edit"), n.id);

    // Acciones edición
    const bSave = nqs("#btn-save-noticia");
    const bCancel = nqs("#btn-cancel-noticia");
    if (bSave && !bSave._b) {
      bSave._b = true;
  ...
  async function saveNoticia() {
    const body = {
      ...
      titulo: nval("f_titulo"),
      desc_uno: nval("f_desc_uno"),
      desc_dos: nval("f_desc_dos"),
      estatus: nnum("f_estatus"),
    };
