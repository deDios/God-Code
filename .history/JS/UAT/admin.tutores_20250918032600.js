...
(() => {
  ...
  function toast(m,t="info",ms=2200){ if (window.gcToast) return window.gcToast(m,t,ms); console.log(TAG, "toast["+t+"]: ", m); }
  ...
  async function gcAskImageUpload({ file, title="Vista previa de imagen", detailsText="" }) {
    // Si tu app define un preview, úsalo:
    const openers = [
      window.gcImagePreview,        // preferente
    ...
    if (openers.length){
      return await new Promise((resolve) => {
        // contrato esperado: onConfirm/onCancel
        try {
          openers[0]({
            file, title, detailsText,
    ...
    return await new Promise((resolve) => {
      ...
      m.innerHTML = `
        <div style="position:fixed;inset:0;background:rgba(0,0,0,.5);display:grid;place-items:center;z-index:9999;">
          <div style="background:#fff;width:min(960px,92vw);max-height:92vh;border-radius:12px;overflow:hidden;box-shadow:0 12px 24px rgba(0,0,0,.2);">
            <div style="padding:16px 18px;border-bottom:1px solid #eee;font-weight:700;">${esc(title)}</div>
            <div style="display:grid;grid-template-columns:1fr 280px;gap:16px;padding:16px;max-height:calc(92vh - 120px);overflow:auto;">
              ...
              <div>
                ...
                <div style="line-height:1.6;">
                  ...
                  <div><strong>Peso:</strong> ${(file.size/1024).toFixed(1)} KB</div>
      ...
      m.querySelector("#p-cancel").onclick = () => { m.remove(); resolve(false); };
      m.querySelector("#p-ok").onclick     = () => { m.remove(); resolve(true);  };
    });
  }
  ...
  // binds una vez (overlay cierra el drawer visible)
  (function bindDrawerOnce(){
    qsa("#drawer-tutor-close").forEach(b=>{ if (!b._b){ b._b=true; b.addEventListener("click", closeTutorDrawer); }});
    const ov = qs("#gc-dash-overlay");
    if (ov && !ov._b){ ov._b=true; ov.addEventListener("click", () => {
        // si está abierto el mini, ciérralo; si no, el de tutor
  ...
  function renderDrawer(dataset){
    ...
    setTimeout(async () => {
      ...
      // Imagen (edición / crear) con lápiz y PREVIEW
      if (isEdit || isCreate){
        const tid = Number(t.id || item?.id || 0);
        ...
        if (imgEdit){
          ...
          else if (S.tempNewTutorImage instanceof File) { imgEdit.src = withBust(URL.createObjectURL(S.tempNewTutorImage)); }
        ...
        if (pencil && !pencil._b){
          ...
          pencil.addEventListener("click", async () => {
            ...
            input.addEventListener("change", async ()=>{
              ...
              // Abre preview; si el usuario confirma Subir:
              const confirmed = await gcAskImageUpload({ file, title: "Vista previa de imagen" });
              if (!confirmed) return;

              const info = qs("#tutor-img-info");
              const u = URL.createObjectURL(file);
              if (!tid){
                // Crear: aún no hay id → guardar para subir al guardar
  ...
  async function saveNew(){
    ...
    if (newId){
      ...
      if (re){
        state.currentDrawer = { type:"tutor", id:newId, mode:"view" }; // ✅ vuelve a vista
  ...
  async function saveUpdate(item){
    ...
    if (re){
      state.currentDrawer = { type:"tutor", id:item.id, mode:"view" }; // ✅ vuelve a vista
      openTutorDrawer("Tutor · " + re.nombre, renderDrawer({ id:String(re.id) }));
    }
  }
  ...
  async function renderTutorCursosChips(tutorId, containerSel="#tutor-cursos"){
    ...
    try{
      ...
      const statusCls = v => {
        const n = Number(v);
        if (n===1) return "s-1";      // Activo (verde)
        if (n===2) return "s-2";      // Pausado (naranja)
        if (n===3) return "s-3";      // Terminado (gris)
        if (n===4) return "s-3";      // En curso → usa s-3 por tu CSS
        return "s-0";                 // Inactivo/Cancelado (rojo/gris)
      };
      ...
      // click → mini drawer (si no está, se crea al vuelo). Si no quieres mini, puedes llamar a window.openCursoView(cid)
  ...
  function openCursoMiniDrawer(data){
    ...
    try { tutorLabel = window.gcUtils?.mapLabel?.(window.__CursosState?.maps?.tutores || {}, data?.tutor) || (data?.tutor ?? "-"); } catch {}
    ...
    aside.hidden = false; aside.setAttribute("aria-hidden","false"); aside.classList.add("open");
    if (overlay){ overlay.classList.add("open"); overlay.hidden = false; overlay.setAttribute("aria-hidden","false"); }
  }
