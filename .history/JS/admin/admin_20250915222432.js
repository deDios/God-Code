...
/* === END GC PATCH === */
  ...
  function onRouteChange() {
    enforceRouteGuard(); const hash = window.location.hash || (isAdminUser ? "#/cursos" : "#/cuentas"); state.route = hash; state.page = 1; qsa(".gc-side .nav-item").forEach(a => { const active = a.getAttribute("href") === hash; a.classList && a.classList.toggle("is-active", active); a.setAttribute("aria-current", active ? "page" : "false") });
  ...
  function mediaUrlsByType(type, id) { const nid = Number(id); if (type === "noticia") return [`/ASSETS/noticia/NoticiasImg/noticia_img1_${nid}.png`, `/ASSETS/noticia/NoticiasImg/noticia_img2_${nid}.png`]; if (type === "curso") return [`/ASSETS/cursos/img${nid}.png`, `/ASSETS/cursos/img${nid}.jpg`, `/ASSETS/cursos/cursos_img${nid}.png`, `/ASSETS/cursos/cursos_img${nid}.jpg`]; if (type === "tutor") return [`/ASSETS/tutor/tutor_${nid}.png`]; if (type === "usuario") return [`/ASSETS/usuario/usuarioImg/img_user${nid}.png`]; return [] }
  ...
  async function requireCourseImage(id) {
    try {
      const arr = mediaUrlsByType("curso", id);
      return (async () => {
  ...
  function renderPreviewUI(cardEl, file, onConfirm, onCancel) {
    const url = URL.createObjectURL(file), drawer = qs("#gc-drawer"), overlayBG = qs("#gc-dash-overlay"); const prev = { pe: drawer?.style?.pointerEvents, fl: drawer?.style?.filter, dz: drawer?.style?.zIndex, oz: overlayBG?.style?.zIndex, aria: drawer ? drawer.getAttribute("aria-hidden") : null, hadInert: drawer?.hasAttribute ? drawer.hasAttribute("inert") : false };
  ...
  function bindUI() {
    ...
    const addBtn = qs("#btn-add"); if (addBtn) addBtn.addEventListener("click", async () => {
  ...
  async function loadCursos() {
    ...
    try {
      const [e1, e0, e2, e3, tmap, pmap, cmap, calmap, temap, ammap] = await Promise.all([postJSON(API.cursos, { estatus: 1 }), postJSON(API.cursos, { estatus: 0 }), postJSON(API.cursos, { estatus: 2 }), postJSON(API.cursos, { estatus: 3 }), getTutorsMap(), getPrioridadMap(), getCategoriasMap(), getCalendarioMap(), getTipoEvalMap(), getActividadesMap()]);
  ...
  async function openCreateNoticia() {
    ...
    const estSel = statusSelect("f_estatus", 1, "noticias");
    ...
    <div class="media-card">
      ...
      <div class="media-meta">
        <div class="media-label">Imagen ${pos}</div>
    ...
    <div class="field">
      ...
      <div class="value"><input id="f_tit" type="text" placeholder="Título de la noticia"/></div>
    </div>
    <div class="field">
      <div class="label">Descripción (1) <span class="req">*</span></div>
      <div class="value"><textarea id="f_desc1" rows="3" placeholder="Texto principal"></textarea></div>
    </div>
    <div class="field">
      <div class="label">Descripción (2) <span class="req">*</span></div>
      <div class="value"><textarea id="f_desc2" rows="3" placeholder="Texto adicional"></textarea></div>
    </div>
    <div class="field">
      <div class="label">Estatus</div>
      <div class="value">${estSel}</div>
    </div>
    <div class="field">
      <div class="label">Imágenes <span class="req">*</span></div>
      <div class="value">
        <div id="create-media-noticia" class="media-grid">
          ${imgCard(1)}${imgCard(2)}
        </div>
        <div class="hint" style="color:#666;margin-top:.35rem;">Debes subir <strong>ambas</strong> imágenes antes de guardar.</div>
      </div>
    </div>
  `;
    ...
    // 2) habilitar inputs y UX focus
    disableDrawerInputs(false);
    setTimeout(() => qs("#f_tit")?.focus(), 0);

    setTimeout(() => {
      const pick = (pos) => {
        const btn = qs(`#create-news-edit-${pos}`);
        const img = qs(`#create-news-thumb-${pos}`);
        const wrap = qs("#create-media-noticia");
        if (btn && img && wrap) {
          btn.addEventListener("click", () => {
      ...
      qs("#btn-save")?.addEventListener("click", async () => {
        ...
        try {
          const titulo = (qs("#f_tit")?.value || "").trim();
          const desc1 = (qs("#f_desc1")?.value || "").trim();
          const desc2 = (qs("#f_desc2")?.value || "").trim();
          const payload = {
            titulo,
            desc_uno: desc1,
            desc_dos: desc2,
  ...
  function openCursoLightFromTutor(tutorId, curso) {
    const nombre = curso?.nombre || ("Curso #" + curso?.id); const body = `
  ...
  `;
    ...
    setTimeout(() => {
      const cont = qs("#media-curso-lite"); if (cont) mountReadOnlyMedia({ container: cont, type: "curso", id: Number(curso.id), labels: ["Portada"], editable: false });
      qs("#btn-back-tutor")?.addEventListener("click", (e) => { e.preventDefault(); qs("#drawer-body").innerHTML = renderTutorDrawer({ id: String(tutorId) }); const it = state.data.find(x => String(x.id) === String(tutorId)); qs("#drawer-title").textContent = "Tutor · " + (it ? it.nombre : "") })
    }, 0)
  }
  ...
  async function loadUsuarios() {
    ...
    if (hdr) {
      ...
      let c3 = hdr.querySelector(".col-fecha");
      let c4 = hdr.querySelector(".col-status");

      if (c1) c1.textContent = "Nombre";

      // Reusa c2 como correo
      if (c2) {
        c2.textContent = "Correo";
        c2.classList.add("col-correo");
        c2.classList.remove("col-tutor", "col-tipo");
      }

      // Reusa c3 como teléfono
      if (c3) {
        c3.textContent = "Teléfono";
        c3.classList.add("col-telefono");
      }

      // Asegura columna de status
      if (!c4) {
    ...
    qs(".tt-title") && (qs(".tt-title").textContent = "Usuarios:");
  ...
  function drawUsuarios() {
    ...
    renderList(rows, {
      matcher: q => {
        const k = norm(q);
        return it => norm(it.nombre || "").includes(k) || norm(it.correo || "").includes(k) || norm(it.telefono || "").includes(k)
      },
      // ahora 4 columnas: nombre/correo/teléfono/status
      desktopRow: it => `
      ...
      drawerTitle: d => {
        ...
        return it ? ("Usuario · " + (it.nombre || it.correo || ("#" + it.id))) : "Usuario"
      },
...
/* ====================== Suscripciones ====================== */
  async function loadSuscripciones() {
    ...
    try {
      const [e1, e0, e2, e3, cMap, uMap] = await Promise.all([postJSON(API.inscripciones, { estatus: 1 }), postJSON(API.inscripciones, { estatus: 0 }), postJSON(API.inscripciones, { estatus: 2 }), postJSON(API.inscripciones, { estatus: 3 }), getCursosMapAnyStatus(), getUsuariosMapAnyStatus()]);
